...
- Human endpoints use a hashed bearer API token in the initial self-hosted release. Roles are `owner`, `admin`, `auditor`, and `viewer`. Production identity-provider/WebAuthn support replaces token issuance without changing authorization rules.
...
| `/organizations/:org/devices` | GET/POST | viewer/admin | list or enroll devices |
