...
Status: partially qualified on 2026-08-13. The 13-case Chromium/virtual-WebAuthn matrix and browser-artifact scanner pass. The live-process lane also passes through PostgreSQL 17 with `verify-full` TLS, real Cloud and production-built Console BFF processes behind trusted HTTPS, Console session bootstrap, pending-state read, bundle fetch, P-256 signed ACK, exact duplicate ACK, conflicting ACK rejection, and the final applied-state read. CI runs the fixture through one cleanup-safe command and rejects a skipped lane as success. The remaining P0-B gap is to run the complete browser role/recent-auth matrix against these live processes and to bind the resulting artifacts to the same qualification run.
