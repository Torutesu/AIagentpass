...
Status: partially qualified on 2026-08-13. The 13-case Chromium/virtual-WebAuthn matrix and browser-artifact scanner pass, and the exact signed-ACK state machine passes against real PostgreSQL. These are supporting lanes, not the P0-B exit gate: the same browser and signed-ACK flow still must run through live Console BFF and Cloud API processes over trusted HTTPS backed by PostgreSQL.
