...
4. `P0-C4 destructive lifecycle qualification` (driver trust adapter, machine runtime, and 16/16 physical procedures implemented in software): the repository has all 16 fixed entrypoints, an exact one-time assignment of the 20 tests, a fail-closed root-owned/digest-pinned machine runtime, and executable procedures spanning installation, identity, Secure Enclave/Cloud possession, real Claude Code/Cursor signing, authority reduction, audit observation, environmental recovery, upgrade, uninstall/reinstall, and purge. Exact installation mints an exclusive candidate checkpoint over all six release-attested code identities, and every later scenario re-verifies it. A completeness gate now rejects a missing, non-executable, placeholder, non-runtime, or non-checkpoint-bound scenario. Operational qualification remains open: provision the protected Apple Silicon and Intel T2 runners, supply the root-owned scenario inputs and production endpoints, then run all 20 tests against the same notarized PKG. Exit: all 20 tests pass once on each hardware class; retained evidence is reviewed and archived.
