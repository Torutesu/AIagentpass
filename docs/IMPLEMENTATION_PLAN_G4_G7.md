...
4. `P0-C4 destructive lifecycle qualification` (driver trust adapter implemented; physical scenarios open): the repository now has all 16 fixed entrypoints, an exact one-time assignment of the 20 tests, and a fail-closed runtime that permits only root-owned digest-pinned scenarios and requires their result to bind the exact PKG, source commit, and Team ID. Next, implement and provision the real install, Secure Enclave, Claude Code, Cursor, revoke, emergency-stop, crash, reboot, sleep/wake, network/clock, upgrade, uninstall/reinstall, and purge scenario executables. Exit: all 20 tests pass once on each hardware class against the same PKG; retained evidence is reviewed and archived.
