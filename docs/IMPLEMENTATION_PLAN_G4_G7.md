...
The detailed forward sequence from the current three-of-sixteen physical-scenario checkpoint is maintained in [`FORWARD_IMPLEMENTATION_PLAN.md`](./FORWARD_IMPLEMENTATION_PLAN.md). It defines the remaining scenario contracts, dependency graph, parallel lanes, evidence, and merge gates.
...
1. `P0-C1 candidate binding` (software implemented): `release-candidate.yml` now generates the migration manifest and release attestation before signing, includes both in the manifest-bound artifact, and cannot publish. Protected immutable Cloud image digest and canonical signer-version inputs are mandatory. Operational exit still requires one real signed candidate run.
2. `P0-C2 protected hardware lanes` (workflow and initial provisioning implemented; runner registration open): the dedicated workflow verifies the exact candidate and separates Apple Silicon and Intel T2 into protected environments with independent operator keys and root-owned drivers. The production-locked provisioner now installs the exact driver/runtime/scenario inventory atomically under `/opt/agentpass/p0c`, with root ownership, digest-pinned configuration, post-install verification, and no in-place replacement. Operational exit still requires provisioning and registering both physical runners and executing the real gate set.
3. `P0-C3 aggregate promotion` (software implemented, execution open): a secret-free aggregate job validates both reports against the signed release and external approved-operator policy. `promote-qualified-release.yml` then replays release and aggregate verification on hosted macOS, compares the retained summary byte-for-byte, verifies the annotated source tag, and alone may create the public release. Missing or mismatched qualification evidence cannot be bypassed by the candidate workflow.
4. `P0-C4 destructive lifecycle qualification` (driver trust adapter, machine runtime, and 3/16 physical scenarios implemented): the repository has all 16 fixed entrypoints, an exact one-time assignment of the 20 tests, a fail-closed root-owned/digest-pinned machine runtime, and real procedures for exact PKG/Gatekeeper installation, launchd/XPC verification, and Secure Enclave enrollment/non-exportability/possession. Exact installation now mints an exclusive candidate checkpoint over all six release-attested code identities, and every implemented later scenario re-verifies that checkpoint before and after use. The service requires the fixed enrollment-key tag and an exact Developer ID/Team ID/approval-entitlement peer requirement. A strict four-role negative-probe manifest exists, but physical signed probes remain open. Next, implement the remaining Claude Code, Cursor, Cloud/audit, policy, revoke, emergency-stop, crash, reboot, sleep/wake, network/clock, upgrade, uninstall/reinstall, purge, and negative-XPC execution procedures. Exit: all 20 tests pass once on each hardware class against the same PKG; retained evidence is reviewed and archived.
