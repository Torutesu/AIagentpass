...
4. `P0-C4 destructive lifecycle qualification` (driver trust adapter, machine runtime, and 3/16 physical scenarios implemented): the repository has all 16 fixed entrypoints, an exact one-time assignment of the 20 tests, a fail-closed root-owned/digest-pinned machine runtime, and real procedures for exact PKG/Gatekeeper installation, launchd/XPC verification, and Secure Enclave enrollment/non-exportability/possession. The service now rejects any ControlBundle device-key tag other than the fixed enrollment tag and exposes a non-creating public qualification snapshot. Next, implement the remaining Claude Code, Cursor, Cloud/audit, policy, revoke, emergency-stop, crash, reboot, sleep/wake, network/clock, upgrade, uninstall/reinstall, purge, and negative-identity procedures. Exit: all 20 tests pass once on each hardware class against the same PKG; retained evidence is reviewed and archived.
