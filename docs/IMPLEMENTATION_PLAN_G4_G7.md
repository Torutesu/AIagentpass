...
2. `P0-C2 protected hardware lanes` (workflow implemented, provisioning open): the dedicated workflow verifies the exact candidate and separates Apple Silicon and Intel T2 into protected environments with independent operator keys and root-owned drivers. Operational exit requires registering both runners and executing the real gate set.
