...
G3.1 repository code is present as a foundation, but G3 is not complete until G3.2 through G3.6 pass. In particular, hosted runtime must not be switched to these repositories before the forward-only schema closes the documented fail-closed gaps.
