...
Immediate status on 2026-08-14: the deterministic signed-manifest-v3-derived qualification configuration provision/restore implementation and its attack tests are complete locally. The P0-C runner provisioner now installs that tool and its two verification dependencies as one root-owned, digest-manifested tree, and each hardware workflow lane fails before qualification when the protected tree differs from the trusted workflow commit. Release manifest schema v3 binds the deterministic universal controller archive, canonical identity document, Team ID, bundle ID, exact entitlement digest, ordered arm64/x86_64 CDHashes, designated requirements, authorization requirements, and separate accepted-and-stapled notarization evidence. The remaining open boundary is actual privileged invocation and unconditional restore around launchd reload, followed by the real Developer ID identity matrix, six physical scenarios, and retained workflow evidence. Checked-out runner-owned JavaScript must never be executed with `sudo`; production invocation must use the root-owned protected installed copy.
...
2. **Root install/restore wrapper and workflow wiring.** Add the minimal privileged wrapper that calls the deterministic provisioner, reopens the installed file, reloads launchd, and restores the exact pre-run configuration in every success, failure, signal, and timeout path. The P0-C workflow must derive architecture from the protected runner and must not accept Team ID, CDHash, scenario, phase, or candidate identity from an untrusted dispatch input. Acceptance is idempotent setup/teardown with the listener proven unreachable afterward.
