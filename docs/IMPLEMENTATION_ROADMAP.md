...
Checkpoint on 2026-08-14: the slice-1 local trust foundation is implemented. The service now pins the active controller slice CDHash in the root-only qualification configuration and compiles a Team ID, bundle ID, dedicated-entitlement, and CDHash XPC requirement. The controller is packaged as a deterministic, normalized, no-overwrite universal tar archive. A separate closed identity contract binds the archive digest and size, Team ID, bundle ID, exact entitlement digest, and ordered arm64/x86_64 CDHashes and designated requirements. This does not yet make the release binding complete: release-manifest v3 integration, controller notarization evidence, promotion allowlists, protected-runner provisioning, and the physical positive/negative identity matrix remain open.
...
The next coding commit is the remaining slice-1 release integration: add a distinct external-controller section to release manifest v3, sign and independently verify the controller identity contract, carry the correct architecture CDHash into each protected lane's root configuration, and update release/promotion workflow allowlists and artifact publication. Real certificate, notarization, protected runner, KMS/HSM, and independent-review work are explicit external gates and must never be represented by simulated or ad-hoc evidence.
