...
Immediate status on 2026-08-14: the deterministic signed-manifest-v3-derived qualification configuration provision/restore implementation and its attack tests are complete locally. A root orchestration core now serializes runs with a protected exclusive lock, restarts and mechanically observes the fixed launchd service, requires a terminable execution handle, disarms/removes the controller candidate/restores/restarts on every modeled result, signal, timeout, and failure path, proves the qualification listener unavailable, and exposes a separately gated stale-run recovery operation. After pinned release-v3 verification, a protected materializer derives the controller candidate only from the installed root configuration and signs it with a per-run ephemeral Ed25519 key; no release private key is present on the hardware runner and no private key is written to disk. The P0-C runner provisioner installs these components and their verification dependencies as one root-owned, digest-manifested tree, and each hardware workflow lane fails before qualification when the protected tree differs from the trusted workflow commit. Release manifest schema v3 binds the deterministic universal controller archive, canonical identity document, Team ID, bundle ID, exact entitlement digest, ordered arm64/x86_64 CDHashes, designated requirements, authorization requirements, and separate accepted-and-stapled notarization evidence. The remaining open boundary is the fixed installed scenario driver and its real Agent request/listener probes, followed by privileged workflow invocation, the real Developer ID identity matrix, six physical scenarios, and retained workflow evidence. Checked-out runner-owned JavaScript must never be executed with `sudo`; production invocation must use the root-owned protected installed copy.

| Order | Slice | Concrete output | Verification / exit gate | Parallel work allowed |
| --- | --- | --- | --- | --- |
| 1 | N3-E2b-1b protected root provisioning | Add one fail-closed provisioner that verifies the signed v3 manifest, selects only the current hardware architecture's controller CDHash, derives candidate/source/identity/run digests, and atomically patches the installed root-owned `0600` service configuration without replacing unrelated production fields. Add an equally strict teardown/restore operation. | Unknown fields, wrong architecture, caller-supplied CDHash, raw run ID persistence, symlink/hardlink/path swap, unsafe ownership/mode, stale expiry, partial qualification block, overwrite, and interrupted publish all fail closed. Reopening the installed bytes proves exact canonical content before launchd is touched. | Threat-model and fault-matrix tests may proceed in parallel; the service configuration schema and writer remain a single-owner boundary. |
...
| 4 | N3-E2b-2 evidence and N3-E2c aggregation | Emit six canonical secret-free evidence records, independently verify them, sign each lane report, and aggregate unchanged Apple silicon/Secure Enclave and Intel/T2 results. Bind release manifest v3 and the external controller identity/notarization evidence into every level. | Reports bind source, PKG and controller archive digests, all product and controller identities, Team ID, notarization ticket, Cloud/schema/signer versions, OS/hardware/boot facts, and scenario transitions. Any edit, omission, cross-lane mismatch, skipped scenario, or unapproved operator blocks promotion. | Evidence verifier and protected-runner operations can proceed in parallel once step 1 contracts freeze. |
...
2. **Fixed installed scenario driver and workflow invocation.** The root orchestration core and protected installation are implemented. Add the candidate-bound controller materialization, fixed six-scenario driver, real Agent request, disarm/listener probes, and the minimal privileged workflow entrypoint. The workflow must derive architecture from the protected runner and must not accept Team ID, CDHash, phase, controller identity, or candidate identity from an untrusted dispatch input. Acceptance is idempotent setup/teardown with the driver process terminated and listener proven unreachable afterward.
