...
- P1-A/B: PostgreSQL membership mutations now lock authority, enforce exact versions, consume pending recent-auth challenges, revoke affected Human sessions and attributed PostgreSQL capabilities, and commit audit/outbox records atomically. Capability issuance records the exact signed-statement hash and membership version; unexpired durable revocations are merged into the next signed ControlBundle and are enforced by the native broker. Push-triggered refresh and full device/agent/policy PostgreSQL migration remain open, so revocation visibility is bounded by the authenticated native refresh interval rather than being instantaneous on an already-installed bundle.
- P1-C: organizations, members, invitations, credentials, and sessions use versioned HMAC-SHA256 cursors bound to resource, tenant, member, and direction. PostgreSQL ordering is normalized to millisecond precision, credential byte IDs use a deterministic SHA-256 anchor, and real-PostgreSQL traversal covers credentials and sessions. Activity has a dedicated Console cursor secret, deterministic cross-device ordering, tamper/scope protection, and concurrent-newer-insert coverage for bounded unsaturated windows. It fails closed when any upstream stream reaches 500 events because the Cloud audit endpoint still lacks an authoritative cursor contract.
...
- P1-E: protected Organization, Security, and WebAuthn BFF routes forward only the allow-listed Human session cookie, CSRF proof, and operation-specific control headers. They reject missing/external origins, redirects, duplicate session cookies, unexpected `Set-Cookie`, and cacheable responses. The old SIWC + Cloud bearer session bootstrap is disabled outside explicit development/test mode; a production-grade verified identity-to-session exchange is therefore still required before hosted login works.
...
| --- | --- | --- | --- |
| G1 — production session bootstrap | Add a Cloud endpoint that accepts a cryptographically verified OIDC/SIWC assertion through a named identity-provider adapter, resolves immutable `(provider, subject)` membership, rotates the Human session cookie, and returns only CSRF/session metadata. Remove production use of `AGENTPASS_CLOUD_TOKEN` and `AGENTPASS_OPERATOR_USER_IDS` from Console login and operator data routes. | Freeze issuer, audience, nonce, redirect/origin, expiry, and error semantics in Human OpenAPI; add provider-login nonce/state storage and indexes if the provider does not supply replay-safe exchange semantics. | Production-mode BFF-to-Cloud E2E proves forged headers, stale assertions, nonce replay, fixation, open redirects, inactive membership, and cross-organization substitution fail before session issuance. Browser assets, responses, and logs contain no provider token or Cloud bearer. |
| G2 — Activity source pagination | Add a Cloud audit-list cursor over immutable `(device_timestamp, device_id, event_id)` ordering with tenant/device scope and `limit + 1`; return a stable `next_cursor`. Make the Console merge pages without a fixed 500-event completeness assumption, then retire the temporary saturated-window `409`. | Extend control-plane OpenAPI/schema and repository interface; add PostgreSQL indexes for tenant/device/timestamp/event tuple. | Real PostgreSQL tests traverse multiple devices and more than 500 rows under concurrent inserts with no duplicate/omission; tampered, cross-device, cross-tenant, and expired cursors fail closed. |
| G3 — PostgreSQL control-plane cutover | Move devices, enrollments, agents, policies, revocations, capabilities, bundle heads/ACKs, audit events, idempotency, replay state, and rate limits from the file store to tenant-qualified PostgreSQL repositories. Keep the file store only behind an explicit evaluation profile. | Add forward-only migrations, repository conformance fixtures, startup schema/version checks, and transaction isolation/locking decisions per mutation. | Two Cloud instances pass concurrent issue/revoke/emergency-stop/ACK/idempotency/replay tests; process restart and instance switching preserve authority; cross-tenant probes are non-disclosing. |
