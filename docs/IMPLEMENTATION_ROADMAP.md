...
- P1-E: protected Organization, Security, and WebAuthn BFF routes forward only the allow-listed Human session cookie, CSRF proof, and operation-specific control headers. They reject missing/external origins, redirects, duplicate session cookies, unexpected `Set-Cookie`, and cacheable responses. Production session bootstrap now uses the fixed compact Ed25519 Console assertion, a pinned Cloud public key, durable one-time JTI consumption, and an exact active `membership_id` insert; the old bearer bridge is explicit development/test compatibility only. Full adversarial browser-to-Cloud E2E remains open.
