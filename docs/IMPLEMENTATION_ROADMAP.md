...
Checkpoint on 2026-08-14: the local release-binding slice is implemented. Release manifest schema v3 now has one closed `external_qualification_controller` section that binds the deterministic universal controller archive, canonical identity document, Team ID, bundle ID, exact entitlement digest, ordered arm64/x86_64 CDHashes, designated requirements, authorization requirements, and separate accepted-and-stapled notarization evidence. Manifest generation, signing, independent verification, staging, release-candidate publication, P0-C consumption, promotion allowlists, hardware-template generation, and hardware-report validation all enforce the same binding. The macOS release verifier extracts the archive with an independent strict USTAR parser, re-inspects the signed app, re-collects the canonical identity, and repeats codesign, stapler, and Gatekeeper checks. This is local implementation evidence only: protected root provisioning, real Developer ID profiles, physical runners, and retained positive/negative XPC evidence remain open.
