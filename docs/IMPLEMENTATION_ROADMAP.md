...
| G3 — PostgreSQL control-plane cutover (core implementation complete 2026-08-13; operations open) | Hosted runtime uses tenant-qualified PostgreSQL repositories for devices, enrollments, agents, policies, revocations, capability reservations, bundle heads/ACKs, audit/admin-audit, idempotency, device replay state, and authenticated organization rate limits. The file store is available only through the explicit evaluation profile. | Forward-only migration `0011_control_plane_hosted_cutover.sql` adds version/actor/completion/bundle/audit/shared-control state and atomic nonce/rate/prune functions. Runtime profiles reject implicit, mixed, partial, or file-backed hosted configuration. Capability retry identity and nonce are purpose-separated HMAC derivations; only the nonce digest is stored. | Real PostgreSQL applies all 11 migrations. Two pools pass concurrent capability replay, nonce consumption and shared rate decisions; bundle sequences remain monotonic across instances; revocations survive instance switching; process restart preserves policy/capability authority; cross-tenant list probes return no rows. G3.6 staged cutover/rollback, restore comparison, readiness, and operational metrics remain before the G3 exit gate. |
...
| G3.6 — cutover operations | Add preflight, migration, readiness, drain, rollback, and evaluation-import guidance. Expose secret-free health metrics for schema version, pool saturation, lock waits, replay denials, rate limits, stale ACKs, and audit gaps. | A staged cutover and rollback rehearsal preserves committed authority. Backup/restore comparison covers row counts plus cryptographic heads and hashes, not only service availability. | G3.5 |
...
G3.1 through G3.5 are implemented and qualified against the forward-only schema. G3 is not exit-gate complete until G3.6 proves staged cutover/rollback, tenant-consistent backup/restore, readiness/drain behavior, and secret-free operational metrics.
