...
The signed helpers expose the low-level offline ceremony without starting XPC. The client supports `bootstrap-approval-create TAG` and `bootstrap-sign TAG`; it derives exactly one `*.dev.agentpass.approval-keys` entitlement, creates/loads a user-presence generation-1 key, strictly decodes the canonical statement, displays its role/fingerprint/head, and signs it. The service supports root-only `--bootstrap prepare-approval|commit-approval|prepare-service|commit-service|status --config PATH`, accepts bounded exact JSON on stdin, and returns the lifecycle sequence, head, per-role state, and fingerprints. `agentpass setup continue --execute` composes these primitives one durable journal state at a time and reconciles an interrupted partial native ceremony before advancing. Provisioning the initial root-owned service configuration remains an installer/onboarding responsibility.
