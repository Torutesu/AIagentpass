...
5. **N3-E3c-1c — two-phase registry admission.** Replace direct activation with `reserve admission → durable audit_prepared → commit activation`. The reservation holds capacity but grants no signing authority; a crash before durable preparation releases it with no authority, while a crash after preparation follows the authority-free reconciliation path. Commit must verify the exact Lease/local result digest and be the only transition that publishes active authority. Exit: kill tests at reserve, prepared-file fsync/rename/parent-fsync, activation commit, and compensation prove every possible active authority has a prior durable intent and restart never fabricates an activation.
6. **N3-E3c-2 — four coordinator hooks.** Inject an optional qualification-fault consumer into `NativeAgentSessionCoordinator`; the normal dependency is a zero-surface no-op. Consume one armed fault immediately before Cloud consume, after a verified committed consume and before registry activation, after registry activation and before audit append, and after the audited terminal record is durable but before result publication. Kill hooks terminate the daemon only after the controller has atomically marked the phase fired. Exit: table tests prove each hook fires once, an unarmed or mismatched hook changes no behavior, and post-activation failures compensate or disappear on restart without publishing authority.
