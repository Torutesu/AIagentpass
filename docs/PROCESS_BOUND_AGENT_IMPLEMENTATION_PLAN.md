...
Status: active implementation sequence after the M1 physical-qualification slice. As of 2026-08-13, the M2 contract/persistence foundation, hosted Grant/Lease authority, atomic consume audit/outbox and lifecycle revocation, audit-token-derived XPC peer metadata capture, PID-scoped Darwin process and Git worktree observation, split Agent XPC listener, connection-bound one-time bootstrap, strict native Cloud Lease decoder, monotonic deadline boundary, linearizable in-memory lease/budget registry, and process-bound `start/status/close` activation coordinator are implemented. Fixed signing, complete lifecycle invalidation, restart/fault qualification, signed-host physical qualification, adapters, external KMS exercises, and production evidence remain open. Passing unit tests alone does not make a lane production-ready.
...
5. **N3-E3c-1c — staged registry admission; foundation implemented, coordinator cutover open.** The registry now implements `reserve → commit-hidden → publish`: reservations and hidden commits consume capacity but are invisible to status, close, and signing; only exact publication creates usable authority; connection/global invalidation removes every unpublished state; and no reservation survives a fresh registry. Black-box tests cover substitution, idempotence, invalidation, restart, and 100-way admission contention. The coordinator must not use this API until the durable outcome protocol below is complete:
   1. Harden the registry transition API before integration: commit/publish must revalidate the exact connection owner and a live monotonic deadline; add an exact abort operation for pending and hidden states; and prune expired unpublished entries under the registry lock so abandoned work cannot retain capacity indefinitely.
   2. Introduce recovery schema v4, rather than changing closed v3 semantics in place. V4 binds an exact activation transaction digest and closed terminal outcomes: `activated`, `aborted`, and `outcome_unknown`. Existing v3 `audit_prepared` records follow their frozen v3 reconciliation path or quarantine; they are never reinterpreted as v4 commit proof.
...
6. **N3-E3c-2 — coordinator qualification hooks.** Inject an optional qualification-fault consumer into `NativeAgentSessionCoordinator`; the normal dependency is a zero-surface no-op. Closed one-shot hooks cover pre-Cloud consume, verified Cloud commit, admission reserve, durable prepare, hidden commit, durable commit receipt, durable audit, terminal recovery, publication, and encoded pre-reply state. Kill hooks terminate the daemon only after the controller atomically marks the phase fired. Exit: table tests prove each hook fires once, an unarmed or mismatched hook changes no behavior, and no failure can expose unpublished authority.
7. **N3-E3c-3 — audit-fsync and transport boundaries.** Put the audit-fsync fault in the concrete audit writer below the coordinator so a reported success still means durable bytes. Put transport-reply loss in the Agent XPC endpoint after successful result encoding and before invoking the reply closure; it must not call `abortActivation` or reinterpret committed business state. Exit: audit failure revokes local authority and leaves recoverable Cloud evidence; lost reply returns the exact audited result on retry without a second Cloud consume or audit append.
8. **N3-E2b-1 — external signed controller.** Add a separately signed, non-bundled command-line controller with fixed `arm/status/disarm` commands, no arbitrary service/selector/phase inputs, sanitized environment, bounded output, and a dedicated entitlement/profile. Its request is derived from the root configuration and candidate manifest, not caller-provided identity fields. Exit: wrong Team ID, entitlement, UID, candidate/run digest, expiry, phase, replay, and second-controller cases are denied by a real XPC round trip.
