...
3. **N3-E3c-1 — audited terminal recovery record.** Replace `completeAfterLocalActivation` deletion with an atomic `pending → audited` transition after the activation audit fsync. The terminal record contains only the immutable recovery tuple plus session/result/audit digests and expiry; it contains no Lease, proof, path, PID, connection token, credential, or active authority. Exact retry may read the same result, but a restarted daemon must never reconstruct registry authority. Conflicting tuple/result/audit data, rollback, truncation, symlink/hard-link substitution, noncanonical bytes, or expiry fails closed. Exit: crash tests at every file-write/fsync/rename boundary prove one Cloud Session, one activation audit, zero restored authority, and byte-identical retry result.
4. **N3-E3c-2 — four coordinator hooks.** Inject an optional qualification-fault consumer into `NativeAgentSessionCoordinator`; the normal dependency is a zero-surface no-op. Consume one armed fault immediately before Cloud consume, after a verified committed consume and before registry activation, after registry activation and before audit append, and after the audited terminal record is durable but before result publication. Kill hooks terminate the daemon only after the controller has atomically marked the phase fired. Exit: table tests prove each hook fires once, an unarmed or mismatched hook changes no behavior, and post-activation failures compensate or disappear on restart without publishing authority.
5. **N3-E3c-3 — audit-fsync and transport boundaries.** Put the audit-fsync fault in the concrete audit writer below the coordinator so a reported success still means durable bytes. Put transport-reply loss in the Agent XPC endpoint after successful result encoding and before invoking the reply closure; it must not call `abortActivation` or reinterpret committed business state. Exit: audit failure revokes local authority and leaves recoverable Cloud evidence; lost reply returns the exact audited result on retry without a second Cloud consume or audit append.
6. **N3-E2b-1 — external signed controller.** Add a separately signed, non-bundled command-line controller with fixed `arm/status/disarm` commands, no arbitrary service/selector/phase inputs, sanitized environment, bounded output, and a dedicated entitlement/profile. Its request is derived from the root configuration and candidate manifest, not caller-provided identity fields. Exit: wrong Team ID, entitlement, UID, candidate/run digest, expiry, phase, replay, and second-controller cases are denied by a real XPC round trip.
7. **N3-E2b-2 — launchd process-kill driver and evidence collector.** Extend `crash-restart-recovery` to install the exact PKG, provision the root-only qualification file, arm through the controller, invoke the real Agent host, observe PID/start/boot changes, query the authenticated Device API independently, and emit the six canonical evidence records. Teardown must disarm, remove configuration, kick/restart the daemon, prove the listener is unreachable, and scan retained files for secrets even after failure. Exit: all six scenarios pass locally on an eligible signed runner and the independent verifier accepts untouched evidence.
