...
5. **N3-E3c-1c — staged registry admission; foundation implemented, coordinator cutover open.** The registry now implements `reserve → commit-hidden → publish`: reservations and hidden commits consume capacity but are invisible to status, close, and signing; only exact publication creates usable authority; connection/global invalidation removes every unpublished state; and no reservation survives a fresh registry. Black-box tests cover substitution, idempotence, invalidation, restart, and 100-way admission contention. The coordinator must not use this API until the durable outcome protocol below is complete:
   1. Extend recovery v3 with an exact activation transaction digest and closed terminal outcomes: `activated`, `aborted`, and `outcome_unknown`. Never reinterpret a prepared intent as successful activation.
   2. Persist and fsync `audit_prepared`, then commit hidden authority. Persist an exact commit receipt before success auditing. If commit happened but its receipt cannot be proven after a crash, retain an `outcome_unknown` tombstone and never reconstruct authority.
   3. Reconcile an exact audit event using the transaction/commit digest in addition to session/agent/evidence fields. A verified absence may append only the outcome proven by durable state; lookup error, duplicate, substitution, or malformed history quarantines the transaction.
   4. Advance recovery to its audited terminal state, then call `publishActivation`, then publish the XPC result. Audit or terminal-store failure invalidates hidden authority and retains sufficient non-secret evidence for exact retry.
   5. Add deterministic fault tests at reserve, prepared file write/fsync/rename/parent-fsync, hidden commit, commit-receipt durability, audit append/fsync, audited rename/fsync, publish, encoding, and reply loss. Add linearization races for deadline, connection invalidation, capacity, and audit rotation.
