...
The next implementation commits are fixed as follows:

1. **N3-E3a — complete root-owned qualification configuration.** Add an all-or-none configuration block containing the fixed qualification Mach service name, exact controller designated requirement, candidate/source/code-identity digests, run expiry, and an explicit qualification-mode marker. Validate ownership, mode, regular-file/single-link status, canonical encoding, expiry, and installed candidate identity before constructing a listener. Missing, partial, stale, or mismatched configuration leaves no listener and no enabled controller. The ordinary setup, Console, Agent host, and app bundle cannot create this file.
2. **N3-E3b — isolated listener and service bridge.** Register a third listener only when N3-E3a succeeds; authenticate its audit token against the dedicated requirement before exporting the three-selector interface. Map the closed XPC phase to the matching closed scenario, bind every operation to the configured candidate and run digests, return only digest receipts, clear state on connection invalidation/service shutdown, and reject replay, a second arm, wrong receipt, wrong identity, or expiry. The production Agent and management interfaces remain byte-for-byte selector-stable.
3. **N3-E3c — real boundary hooks.** Inject the controller dependency into session activation as optional and disabled by default. Add hooks immediately before Cloud consume, after committed consume/before local activation, after activation/before audit, and after audit/before response. Route audit fsync failure through the audit writer boundary. Implement transport reply loss outside the coordinator so business logic cannot mistake it for rollback. Every kill/loss is one-shot and leaves the existing recovery/compensation rules authoritative.
4. **N3-E2b — physical driver integration.** Extend `crash-restart-recovery` to provision the root-owned qualification configuration on a dedicated runner, arm through the signed external controller, invoke the real Agent host, observe launchd PID/start/boot transitions, query the authenticated Device API independently, and emit the six canonical evidence records. Remove the qualification configuration and verify the listener is unreachable at teardown, including failed runs.
5. **N3-E2c — release qualification.** Build one immutable universal PKG, sign every nested executable and the external controller with the declared identities, notarize/staple, run all six scenarios on Apple silicon/Secure Enclave and Intel/T2, independently sign both reports, and aggregate-verify package/source/Team ID/CodeDirectory equality. N4 remains unavailable if either lane is missing or any report is edited, stale, skipped, or leaks raw output.
6. **N4-A/B — fixed signer and invalidation.** Only after accepted N3-E evidence, connect the existing five-selector Agent `signGitCommit` method to the durable one-signature transaction and then wire the unified invalidation/drain model. Land intent/recovery format changes before runtime wiring; require boundary-fault, replay, 100-way contention, Git verification, revoke/rotation/process-exit races, and restart tests.
7. **N5/N6 — product adapters and onboarding.** Ship the signed launcher, repository-local Git configuration, Claude Code path, then the pinned Cursor adapter/SDK. The Console creates and narrows policy; it never handles the device signing key. Validate a non-engineer flow from enrollment to two unattended verified commits, followed by immediate revocation, with no authority in argv, environment, files, history, logs, or browser storage.
8. **N7 — hosted/release closure.** Complete KMS/HSM key separation and rotation, PostgreSQL expiry/revoke jobs, Organization/role/session/WebAuthn recovery UX, audit projection, deployment/rollback/PITR drills, metrics and alerts, SBOM/provenance, independent security review, and allow-listed production enablement. Release only with both hardware/adapter matrices and zero unresolved critical/high findings.
