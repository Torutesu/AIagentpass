...
The next implementation commits are fixed as follows. N3-E3a and N3-E3b are complete in `6ccecfc`; the first open commit is N3-E3c-1:
...
3. **N3-E3c-1a — complete (audited terminal recovery record and receipt).** `completeAfterLocalActivation` now atomically advances `pending → audited`; the terminal record contains only the immutable tuple plus session/result/audit digests and a bounded expiry. Restart exact-retries the Cloud result but never reconstructs registry authority or emits a second audit after an already visible terminal state. Canonical encoding, v1 rejection, tamper/link/mode denial, idempotence, expiry, and secret-free shape are frozen by Swift and Node contracts.
4. **N3-E3c-1b — prepared-audit reconciliation.** Before activation audit append, durably transition `pending → audit_prepared` with the exact canonical audit-evidence digest and activation-result digest. Extend the concrete audit boundary with an idempotent lookup/append operation keyed by that digest and session ID. After restart, reconcile an exact existing audit record to `audited`, append only when absence is proven against the verified chain, and reject multiple/mismatched records. Only then may the coordinator terminalize or compensate. Exit: kill injection before append, during append fsync, after append fsync, during terminal rename, and after parent fsync proves exactly one activation audit and zero authority restoration.
5. **N3-E3c-2 — four coordinator hooks.** Inject an optional qualification-fault consumer into `NativeAgentSessionCoordinator`; the normal dependency is a zero-surface no-op. Consume one armed fault immediately before Cloud consume, after a verified committed consume and before registry activation, after registry activation and before audit append, and after the audited terminal record is durable but before result publication. Kill hooks terminate the daemon only after the controller has atomically marked the phase fired. Exit: table tests prove each hook fires once, an unarmed or mismatched hook changes no behavior, and post-activation failures compensate or disappear on restart without publishing authority.
6. **N3-E3c-3 — audit-fsync and transport boundaries.** Put the audit-fsync fault in the concrete audit writer below the coordinator so a reported success still means durable bytes. Put transport-reply loss in the Agent XPC endpoint after successful result encoding and before invoking the reply closure; it must not call `abortActivation` or reinterpret committed business state. Exit: audit failure revokes local authority and leaves recoverable Cloud evidence; lost reply returns the exact audited result on retry without a second Cloud consume or audit append.
7. **N3-E2b-1 — external signed controller.** Add a separately signed, non-bundled command-line controller with fixed `arm/status/disarm` commands, no arbitrary service/selector/phase inputs, sanitized environment, bounded output, and a dedicated entitlement/profile. Its request is derived from the root configuration and candidate manifest, not caller-provided identity fields. Exit: wrong Team ID, entitlement, UID, candidate/run digest, expiry, phase, replay, and second-controller cases are denied by a real XPC round trip.
8. **N3-E2b-2 — launchd process-kill driver and evidence collector.** Extend `crash-restart-recovery` to install the exact PKG, provision the root-only qualification file, arm through the controller, invoke the real Agent host, observe PID/start/boot changes, query the authenticated Device API independently, and emit the six canonical evidence records. Teardown must disarm, remove configuration, kick/restart the daemon, prove the listener is unreachable, and scan retained files for secrets even after failure. Exit: all six scenarios pass locally on an eligible signed runner and the independent verifier accepts untouched evidence.
