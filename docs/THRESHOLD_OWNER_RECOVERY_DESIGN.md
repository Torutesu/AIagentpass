...
The public `event_id` is the provider idempotency key. Providers must persistently deduplicate that key and return the exact contract `{accepted:true,duplicate:boolean,idempotency_key}` for accepted or duplicate delivery, or `{accepted:false,duplicate:false,idempotency_key}` only when delivery is definitively rejected. Timeout, transport failure, malformed response, process loss, and PostgreSQL acknowledgement failure are unknown outcomes: after lease expiry the row is quarantined as `uncertain`, never made automatically deliverable. Attempts use bounded exponential backoff with jitter only after explicit rejection. Attempt 100 remains reclaimable after process loss and transitions to `dead_letter` only after an explicit delivery failure; successful delivery can still publish it. Stored diagnostics are stable error codes only—provider messages, response bodies, credentials, destination data, and webhook configuration are never persisted.

Migration 0036 adds a separate linearizable provider lookup over the same durable acceptance ledger. Replicas schedule and claim bounded lookup work with the database clock and `SKIP LOCKED`; the request contains only the exact immutable binding and public idempotency key. An exact positive echoed proof may move `uncertain` to `published` under management-version, confirmation-attempt, state, and binding CAS. Negative, malformed, substituted, timed-out, or lost lookup responses leave the row uncertain and cannot trigger a notification resend. Owner/Admin retry and suppression continue to require resource-bound recent WebAuthn and race safely with automatic confirmation.

Migration 0035 binds each deliverable row to an immutable, secret-free provider
tuple: a canonical binding identifier, positive key version, and random
32-byte deployment digest. The digest is explicitly configured and is never
derived from the webhook URL. Every replica must use the same tuple; changing
the provider account, destination, or authorization authority requires a new
key version and digest. Workers claim only their exact tuple, legacy unbound
rows remain quarantined, and no retry operation may silently bind an existing
event to a new destination. State transitions are additionally recorded in an
immutable per-event hash chain that is separate from the administrator audit
authority.
