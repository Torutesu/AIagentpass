...
const HANDOFF_KEYS = Object.freeze(["version", "correlation_id", "nonce", "invitation"]);
...
export async function createBrowserCliHandoff(options = {}) {
  ...
  const handle = Object.freeze({
    ...
    waitForInvitation: async () => {
      const result = await invitationPromise;
  ...
  async function handleRequest(request, response) {
    ...
    try {
      ...
      const body = await readBoundedBody(request, config.maxBodyBytes, config.requestTimeoutMs);
  ...
  function parseHandoffBody(bytes) {
    let value;
    try { value = parseJsonWithoutDuplicateKeys(bytes.toString("utf8")); } catch { failRequest("request body JSON is invalid", BROWSER_CLI_HANDOFF_ERRORS.INVALID_REQUEST, 400); }
    exactKeys(value, HANDOFF_KEYS, "handoff request");
    if (value.version !== BROWSER_CLI_HANDOFF_VERSION || typeof value.correlation_id !== "string" || !SAFE_TOKEN.test(value.correlation_id) || typeof value.nonce !== "string" || !SAFE_TOKEN.test(value.nonce)) {
...
function readBoundedBody(request, maximum, timeoutMs) {
  return new Promise((resolve, reject) => {
    const chunks = [];
    let size = 0;
    let settled = false;
