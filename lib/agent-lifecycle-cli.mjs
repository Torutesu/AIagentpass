...
import { AGENT_LAUNCH_HANDOFF_FD, AgentLaunchHandoffError, normalizeAgentLaunchAuthority, readAgentLaunchAuthority } from "./agent-launch-handoff.mjs";
...
export const AGENT_LIFECYCLE_HANDOFF_FD = AGENT_LAUNCH_HANDOFF_FD;
...
function validLaunchDescriptor(descriptor) {
  return exactKeys(descriptor, ["agent", "handoff_fd", "operation", "project", "ttl_seconds", "version"])
    ...
    && (descriptor.agent === "claude-code" || descriptor.agent === "cursor")
...
/**
 * Consume the trusted upstream handoff in the CLI and relay it over a fresh
 * anonymous FD3 pipe to the signed Host. This is the production seam: the
 * authority never appears in argv, environment, or a regular file.
 */
export async function launchAgentLifecycleWithHandoff(normalized, options = {}) {
  if (options.platform !== "darwin") return unavailableAgentLifecycle("launch");
  if (!validLaunchDescriptor(normalized)) return publicError("launch", AGENT_LIFECYCLE_HOST_REJECTED, "The lifecycle launch descriptor is invalid");
  ...
  let authority;
  try {
    const read = options.readAuthority ?? (() => readAgentLaunchAuthority({
      fd: AGENT_LIFECYCLE_HANDOFF_FD,
      fstat: options.fstat ?? fs.fstatSync,
      read: options.read ?? fs.readSync,
      close: options.close ?? fs.closeSync
    }));
    authority = normalizeAgentLaunchAuthority(read(), normalized, options.nowMs ?? Date.now());
  } catch (error) {
    if (error instanceof AgentLaunchHandoffError) return publicError("launch", error.code, error.message);
    return publicError("launch", AGENT_LIFECYCLE_HANDOFF_UNAVAILABLE, "The one-time Native Host handoff is not available");
  }

  const run = options.spawn ?? spawn;
  let child;
  try {
    child = run(launcher, [normalized.operation, normalized.project], {
      shell: false,
      env: { PATH: "/usr/bin:/bin:/usr/sbin:/sbin" },
    ...
    return publicError("launch", AGENT_LIFECYCLE_HOST_REJECTED, "The fixed Native Host could not be started");
  }

  const handoffPipe = child?.stdio?.[AGENT_LIFECYCLE_HANDOFF_FD];
  if (!child || !handoffPipe || typeof handoffPipe.end !== "function") {
    authority.bytes.fill(0);
    try { child?.kill?.(); } catch { /* fail closed; the child is not trusted */ }
    return publicError("launch", AGENT_LIFECYCLE_HOST_REJECTED, "The fixed Native Host handoff pipe could not be created");
  }

  const stdout = collectHostOutput(child, MAX_HOST_OUTPUT_BYTES);
  let output;
  try {
    await writeHandoff(handoffPipe, authority.bytes);
    output = await stdout;
  } catch {
    try { child.kill?.(); } catch { /* fail closed */ }
    return publicError("launch", AGENT_LIFECYCLE_HOST_REJECTED, "The fixed Native Host rejected the launch");
  } finally {
    authority.bytes.fill(0);
  }
  if (output.status !== 0 || output.signal) return publicError("launch", AGENT_LIFECYCLE_HOST_REJECTED, "The fixed Native Host rejected the launch");
  const parsed = parseHostOutput(output.stdout);
  if (!parsed) return publicError("launch", AGENT_LIFECYCLE_HOST_REJECTED, "The Native Host returned an invalid launch result");
  return Object.freeze({ ok: true, operation: "launch", status: parsed.status });
}

function writeHandoff(stream, bytes) {
...
function collectHostOutput(child, maximumBytes) {
  return new Promise((resolve, reject) => {
    const chunks = [];
    let total = 0;
    let stdoutError;
    child.stdout?.on?.("data", (chunk) => {
      ...
      if (total > maximumBytes) {
        stdoutError = new Error("Native Host output is oversized");
        try { child.kill?.(); } catch { /* fail closed */ }
        return;
      }
      chunks.push(bytes);
    });
    child.stdout?.once?.("error", (error) => { stdoutError = error; });
    child.once?.("error", (error) => reject(error));
    child.once?.("close", (status, signal) => {
      if (stdoutError) return reject(stdoutError);
      resolve({ status, signal, stdout: Buffer.concat(chunks, total).toString("utf8") });
    });
  });
}
