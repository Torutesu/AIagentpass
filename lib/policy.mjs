...
function isSameRepository(candidate, root) {
  const real = (value) => {
    try { return fs.realpathSync(value); } catch { return path.resolve(value); }
  };
  return real(candidate) === real(root);
}
...
export function evaluateRequest({ policy, cwd, branch = "", remote = "", operation = "git.commit.sign", revoked = false }) {
  ...
  const allowedRepo = (policy.repositories ?? []).some((configured) => isSameRepository(cwd, configured));
...
export function evaluateAgentRequest(context, agent) {
  const globalDecision = evaluateRequest(context);
  if (!globalDecision.allowed || !agent?.scope) return globalDecision;
  const scopedDecision = evaluateRequest({ ...context, policy: { ...agent.scope, session: context.policy.session } });
  return scopedDecision.allowed ? globalDecision : { ...scopedDecision, reason: `agent_scope_${scopedDecision.reason}` };
}
