...
export function verifyNativeCheckpointRecord(record, publicKey, { previousCheckpointHash } = {}) {
  ...
  const statement = record.version === 2 ? {
    ...
    lifecycle_head_hash: record.lifecycle_head_hash
  } : {
    version: record.version,
    created_at: record.created_at,
    entries: record.entries,
  ...
  if (![1, 2].includes(statement.version) || !Number.isFinite(Date.parse(statement.created_at))) throw checkpointError("invalid_statement");
  ...
  if (statement.version === 2 && (!Number.isSafeInteger(statement.key_generation) || statement.key_generation < 1 || !isHash(statement.lifecycle_head_hash))) throw checkpointError("invalid_statement");
