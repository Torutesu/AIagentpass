...
export function enrollAnchorTenant(directory, tenant, auditPublicKey) {
  ...
  const config = { version: identity.algorithm === "ed25519" ? 1 : 2, tenant, checkpoint_algorithm: identity.algorithm, audit_public_key: identity.publicKey, audit_key_fingerprint: identity.fingerprint, enrolled_at: new Date().toISOString() };
...
export function submitAnchorCheckpoint(directory, tenant, checkpoint, now = Date.now()) {
  ...
  const verifiedCheckpoint = verifyTenantCheckpoint(checkpoint, tenantConfig);
...
function verifyTenantRecords(records, tenantConfig, anchorPublicKey, tenant) {
  ...
  for (let index = 0; index < records.length; index += 1) {
    const checkpoint = verifyTenantCheckpoint(records[index].checkpoint, tenantConfig, { previousCheckpointHash: previousCheckpoint });
...
function loadTenant(directory, tenant) {
  ...
  if (![1, 2].includes(config?.version) || config.tenant !== tenant) throw new Error("Anchor tenant configuration is invalid");
...
function verifyTenantCheckpoint(checkpoint, tenantConfig, options = {}) {
  if (tenantConfig.checkpoint_algorithm === "p256-sha256") return verifyNativeCheckpointRecord(checkpoint, tenantConfig.audit_public_key, options);
  if (tenantConfig.checkpoint_algorithm === "ed25519" || tenantConfig.version === 1) return verifyCheckpointRecord(checkpoint, tenantConfig.audit_public_key, options);
