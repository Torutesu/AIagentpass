...
const RECORD_KEYS = Object.freeze([
  ...
  "state", "revision", "created_at", "updated_at", "previous_hash", "record_hash", "evidence", "failure"
]);
...
export class DeviceOnboardingResumeStore {
  ...
  create_prepared(input = {}) {
    assertInputKeys(input, ["release_id", "organization_id", "device_id", "resume_id", "created_at"]);
...
function verifyChain(entries, journal) {
  for (let index = 0; index < entries.length; index += 1) {
    ...
    if (index > 0 && (record.resume_id !== entries[0].resume_id || record.release_id !== entries[0].release_id || record.organization_id !== entries[0].organization_id || record.device_id !== entries[0].device_id)) fail("BINDING_FAILURE", "Resume journal binding changed");
