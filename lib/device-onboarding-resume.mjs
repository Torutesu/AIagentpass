...
export class DeviceOnboardingResumeStore {
  ...
  /**
   * Atomically create the first durable record after an invitation has been
   * received. Only public invitation evidence is retained; the credential,
   * nonce, and signature remain transient. This prevents a restart from
   * getting stuck at a prepared record whose one-shot invitation is gone.
   */
  create_invitation_issued(input = {}) {
    assertInputShape(input, [
      "release_id", "organization_id", "device_id", "invitation_id", "invitation_hash"
    ], [
      "release_id", "organization_id", "device_id", "resume_id", "created_at",
      "recovery_descriptor", "invitation_id", "invitation_hash", "issued_at"
    ]);
    const releaseId = validateReleaseId(input.release_id);
    const organizationId = validatePublicId(input.organization_id, "organization_id");
    const deviceId = validatePublicId(input.device_id, "device_id");
    ...
    return this._withLock(() => {
      if (hasAnyStorage(this)) fail("ALREADY_EXISTS", "Resume state already exists");
      const issuedAt = validateTime(input.issued_at ?? input.created_at ?? this.clock(), "issued_at");
      const record = makeRecord({
        release_id: releaseId,
        organization_id: organizationId,
        device_id: deviceId,
