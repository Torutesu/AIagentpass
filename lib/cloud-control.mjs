...
function extractBundle(value) {
  ...
  if (Object.hasOwn(value, "bundle")) {
    ...
    if (keys.some((key) => !["bundle", "desired_generation", "request_id"].includes(key))) throw new CloudControlError(CLOUD_CONTROL_ERRORS.RESPONSE, "cloud control response contains unknown fields");
    if (value.desired_generation !== undefined && (!Number.isSafeInteger(value.desired_generation) || value.desired_generation < 1)) throw new CloudControlError(CLOUD_CONTROL_ERRORS.RESPONSE, "cloud control response generation is invalid");
