...
export const SETUP_BROWSER_CONNECT_ERRORS = Object.freeze({
  ...
  LOOPBACK_UNAVAILABLE: "ERR_SETUP_BROWSER_CONNECT_LOOPBACK_UNAVAILABLE",
...
const STATIC_MESSAGES = Object.freeze({
  ...
  [SETUP_BROWSER_CONNECT_ERRORS.LOOPBACK_UNAVAILABLE]: "The local browser handoff is unavailable",
...
export async function connectSetupInBrowser(options = {}) {
  ...
  try {
    ...
    throwIfAborted(config.signal);
    try {
      invitation = await handoff.waitForInvitation();
    } catch (error) {
      throw mapHandoffError(error, config.signal);
    }
  } catch (error) {
    primaryError = error instanceof SetupBrowserConnectError ? error : mapHandoffError(error, config.signal);
    throw primaryError;
  } finally {
    if (handoff) {
      try {
  ...
  // Reached only when the primary path produced a value: a close failure on
  // that path must still surface instead of silently succeeding.
  if (closeFailed) throw new SetupBrowserConnectError(SETUP_BROWSER_CONNECT_ERRORS.CLOSE_FAILED);
  return invitation;
}
...
function mapHandoffError(error, signal) {
  ...
  // Listener startup errors include environment-dependent failures such as
  // EPERM/EACCES and port/address availability.  Do not inspect or expose
  // their message/cause: those values can contain local paths or secrets.
  // A single stable code is sufficient for callers to mark the browser path
  // unavailable (and, where appropriate, leave real E2E qualification
  // not_proven) without pretending that the handoff executed.
  if (error?.code === BROWSER_CLI_HANDOFF_ERRORS.LISTENER) return new SetupBrowserConnectError(SETUP_BROWSER_CONNECT_ERRORS.LOOPBACK_UNAVAILABLE);
