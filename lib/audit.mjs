...
export function verifyAuditCheckpoints(publicKey, dir) {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    const { checkpoint_hash: checkpointHash, signature, public_key_fingerprint: fingerprint, ...statement } = record;
    if (statement.version !== 1 || statement.previous_checkpoint_hash !== previous) return invalidCheckpoint(lines.length, index, "checkpoint_chain_invalid");
    if (!Number.isInteger(statement.entries) || statement.entries < 0 || auditHashes[statement.entries] !== statement.head_hash) return invalidCheckpoint(lines.length, index, "audit_head_mismatch");
    if (fingerprint !== publicKeyFingerprint(publicKey)) return invalidCheckpoint(lines.length, index, "public_key_mismatch");
    let signatureValid = false;
    try { signatureValid = crypto.verify(null, checkpointBytes(statement), publicKey, Buffer.from(signature, "base64")); } catch {}
    if (!signatureValid) return invalidCheckpoint(lines.length, index, "signature_invalid");
    const copy = { ...statement, public_key_fingerprint: fingerprint, signature };
    if (digest(copy) !== checkpointHash) return invalidCheckpoint(lines.length, index, "checkpoint_hash_invalid");
