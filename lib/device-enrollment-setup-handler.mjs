...
function controlEvidence(result, controlURL) {
  const publicValue = { organization_id: result.organization_id, device_id: result.device_id, enrollment_id: result.enrollment_id, control_url: controlURL, public_key: result.control.public_key, key_id: result.control.key_id };
  ...
  return async function enrollDevice(context) {
    ...
    if (resumeStore) {
      ...
      if (record.state === "trust_installed") {
        ...
        saveConfig(next);
        resumeStore.acknowledge_control(controlEvidence(result, controlURL));
        record = resumeStore.read();
      }
