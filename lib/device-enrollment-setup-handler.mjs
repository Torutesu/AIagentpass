...
import { createDeviceEnrollmentClient, recoverDeviceEnrollment, verifyDeviceEnrollmentReceipt } from "./device-enrollment-client.mjs";
...
function assertCandidateBinding(binding, expected = {}) {
  ...
  if (binding.version !== 1 || !HASH.test(binding.artifact_sha256) || !/^[0-9a-f]{40}$/u.test(binding.source_commit) || !/^[A-Z0-9]{10}$/u.test(binding.team_id) || !FINGERPRINT.test(binding.device_key_fingerprint) || !RFC3339_UTC.test(binding.expires_at) || !Number.isFinite(expiresAt) || expiresAt <= Date.now()) fail("RESUME_BINDING_MISMATCH", "Recovery candidate binding is invalid or expired");
...
function assertResumeStore(store) {
  ...
  for (const method of ["read", "create_prepared", "create_invitation_issued", "issue_invitation", "record_delivery", "mark_enrollment_uncertain", "reconcile_enrollment", "install_trust", "acknowledge_control"]) {
...
function publicProof(result, descriptor, key) {
  ...
  if (result.request_hash !== descriptor.request_digest || !object(result.possession_receipt)) fail("ENROLLMENT_RECOVERY_UNVERIFIED", "Enrollment recovery did not return the authoritative possession receipt");
  let receipt;
  try {
    receipt = verifyDeviceEnrollmentReceipt(result.possession_receipt, descriptor.verification_public_key, {
      candidateBinding: descriptor.candidate_binding,
      ...
      deviceKeyEpoch: result.device_key_epoch
    });
  } catch { fail("ENROLLMENT_RECOVERY_UNVERIFIED", "Enrollment recovery possession receipt is not authoritative"); }
  if (receipt.key_id !== descriptor.verification_key_id || receipt.algorithm !== descriptor.verification_algorithm || canonicalJson(receipt.statement.control) !== canonicalJson(result.control)) fail("ENROLLMENT_RECOVERY_UNVERIFIED", "Enrollment recovery receipt does not match the returned control trust");
 ...
 */
function createDeviceEnrollmentSetupHandlerInternal(options = {}, { allowLegacy = false } = {}) {
  ...
  if (!allowLegacy && resumeStore === undefined) fail("INVALID_RESUME_STORE", "The D2 enrollment handler requires a durable resume store");
  ...
  return async function enrollDevice(context) {
    ...
    if (resumeStore) {
      ...
      if (record) {
        ...
        const invitationRecord = invitationEvidence(normalizedInvitation, descriptor);
        record = resumeStore.create_invitation_issued({
          release_id: descriptor.candidate_binding.candidate_id,
          ...
          invitation_hash: invitationRecord.invitation_hash,
          issued_at: undefined
        });
        createdHere = true;
      }
    ...
    if (resumeStore) {
      ...
      // Preserve safe recovery for records written by the pre-atomic runtime.
      // New executions start at invitation_issued, so a fresh process never
      // creates a prepared record that depends on a lost one-shot invitation.
...
/**
 * D2 enrollment entry point. Crash-resumable durable state is mandatory for
 * this API; callers must not accidentally fall back to a one-shot POST path.
 */
export function createDeviceEnrollmentSetupHandler(options = {}) {
  return createDeviceEnrollmentSetupHandlerInternal(options);
}

/**
 * Explicit compatibility API for callers that have intentionally accepted
 * the pre-D2 one-shot behavior. This API never accepts a resume store so the
 * compatibility boundary remains visible at every call site.
 */
export function createLegacyDeviceEnrollmentSetupHandler(options = {}) {
  if (options.resumeStore !== undefined) fail("INVALID_RESUME_STORE", "The legacy enrollment handler cannot be combined with a resume store");
  return createDeviceEnrollmentSetupHandlerInternal(options, { allowLegacy: true });
}
