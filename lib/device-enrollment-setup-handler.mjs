...
import { buildEnrollmentCandidateBinding, createV2DeviceEnrollmentClient } from "./device-enrollment-client.mjs";

function fail(code, message) { const error = new Error(message); error.code = code; throw error; }
function object(value) { return value !== null && typeof value === "object" && !Array.isArray(value); }
const INVITATION_KEYS = new Set(["version", "proof_version", "enrollment_id", "organization_id", "device_id", "label", "platform", "candidate_binding", "challenge_id", "nonce", "expires_at", "challenge", "credential", "endpoint", "possession_receipt_verification"]);
...
function strictV2Invitation(value) {
  ...
  if (value.version !== 2 || value.proof_version !== 2 || value.platform !== "macos") fail("INVALID_ENROLLMENT_INVITATION", "A v2 macOS enrollment invitation is required");
...
export function createDeviceEnrollmentSetupHandler(options = {}) {
  ...
  return async function enrollDevice(context) {
    ...
    client ??= createV2DeviceEnrollmentClient({
      proofVersion: 2,
      requireV2: true,
