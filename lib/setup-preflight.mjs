...
// install authority supplies both values after running the existing signed
// release validation primitive; the receipt is public identity evidence only.
export const SETUP_PREFLIGHT_INSTALL_RECEIPT_CONTRACT = Object.freeze({
  ...
  binding: "candidate_id, release_version, and all release digests must match the exact result returned by the existing signed-release verifier"
});
...
function releaseBinding(value, label, code = SETUP_PREFLIGHT_ERROR_CODES.INSTALL_RECEIPT_INVALID) {
  ...
  try { parseReleaseVersion(value.release_version); }
  catch { fail(code, `${label} release_version is invalid`); }
