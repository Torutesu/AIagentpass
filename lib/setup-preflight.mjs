...
export function parseSetupPreflightHandoff(value) {
  exactKeys(value, SETUP_PREFLIGHT_HANDOFF_KEYS, "Setup preflight handoff", SETUP_PREFLIGHT_ERROR_CODES.HANDOFF_INVALID);
  if (value.version !== SETUP_PREFLIGHT_VERSION || value.platform !== SETUP_PREFLIGHT_PLATFORM) {
    fail(SETUP_PREFLIGHT_ERROR_CODES.HANDOFF_INVALID, "Setup preflight handoff identity is invalid");
  }
  boundedString(value.candidate_id, CANDIDATE_ID, "Setup preflight handoff candidate_id", SETUP_PREFLIGHT_ERROR_CODES.HANDOFF_INVALID);
  boundedString(value.device_key_fingerprint, FINGERPRINT, "Setup preflight handoff device key fingerprint", SETUP_PREFLIGHT_ERROR_CODES.HANDOFF_INVALID);
  return Object.freeze({
