...
const ENROLLMENT_KEYS = new Set(["version", "enrollment_id", "organization_id", "device_id", "status", "key_algorithm", "control"]);
const CONTROL_KEYS = new Set(["format_epoch", "issuer", "key_id", "public_key", "bundle_path"]);
...
export function deviceEnrollmentEvidence(result) {
  ...
  return Object.freeze({ organization_id: organizationId, device_id: deviceId, enrollment_id: enrollmentId, key_fingerprint: result.key_fingerprint });
}
...
async function performEnrollment(config) {
  ...
  const evidence = deviceEnrollmentEvidence({ enrollment_id: request.enrollment_id, organization_id: request.organization_id, device_id: request.device_id, key_fingerprint: config.key_fingerprint });
  return Object.freeze({
    status: "enrolled",
...
function validateResponseBinding(value, request, expectedFingerprint) {
  ...
  return {
    ...
    control: { ...control, public_key: controlKey },
    server: {
      version: VERSION,
      enrollment_id: request.enrollment_id,
      organization_id: request.organization_id,
      ...
      control: { format_epoch: 2, issuer: control.issuer, key_id: control.key_id, public_key: controlKey, bundle_path: control.bundle_path },
