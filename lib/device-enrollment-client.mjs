...
async function assertNoCompletedEnrollment(config, request) {
  ...
  if (observed.receipt) throw recoveryProven(observed.receipt, "preflight");
  if (observed.status === 401 || observed.status === 404) return;
  throw recoveryUnproven(observed.status, "preflight");
}
...
async function recoverAfterAmbiguousCompletion(config, request) {
  const observed = await readPossessionReceipt(config, request, undefined, "post");
  if (observed.receipt) throw recoveryProven(observed.receipt, "post");
  throw recoveryUnproven(observed.status, "post");
}

async function requirePossessionReceipt(config, request, deviceKeyEpoch) {
  const observed = await readPossessionReceipt(config, request, deviceKeyEpoch, "post");
...
function recoveryProven(receipt, phase) {
