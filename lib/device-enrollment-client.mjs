...
const RECEIPT_STATEMENT_KEYS = new Set(["version", "enrollment_id", "organization_id", "device_id", "candidate_id", "artifact_sha256", "source_commit", "team_id", "device_key_fingerprint", "device_key_epoch", "challenge_nonce_digest", "issued_at"]);
  ...
  async function enroll() {
    ...
    inFlight = performEnrollment(config).then((result) => {
      ...
      if (config.proof_version === 2) {
        terminalFailure = sanitizeFailure(error);
        throw cloneFailure(terminalFailure);
      }
      throw error;
...
async function performEnrollment(config) {
  ...
  if (config.proof_version === 2) await assertNoCompletedEnrollment(config, request);
  ...
  try {
    ...
    if (config.proof_version === 2) await recoverAfterAmbiguousCompletion(config, request);
    throw error;
  }
  let parsed;
  try {
    ...
    // receipt lookup.
    if (config.proof_version === 2 && response.status === 201) await recoverAfterAmbiguousCompletion(config, request);
    throw error;
  }
  ...
  const possessionReceipt = config.require_receipt
    ? await requirePossessionReceipt(config, request, safeResponse.device_key_epoch)
    : undefined;
...
async function assertNoCompletedEnrollment(config, request) {
  ...
  if (observed.receipt) throw recoveryProven(observed.receipt);
  if (observed.status === 401 || observed.status === 404) return;
 ...
 * A valid receipt proves that Cloud consumed the credential, but it does not
 * carry the control trust roots returned by POST, so the honest result is a
 * terminal recovery error rather than a fabricated enrollment result.
 */
...
async function requirePossessionReceipt(config, request, deviceKeyEpoch) {
  const observed = await readPossessionReceipt(config, request, deviceKeyEpoch);
  if (observed.receipt) return observed.receipt;
  throw recoveryUnproven(observed.status);
}

async function readPossessionReceipt(config, request, deviceKeyEpoch = undefined) {
  let response;
  try {
    response = await requestPossessionReceipt(config, request);
  } catch (error) {
    throw recoveryUnproven(error instanceof DeviceEnrollmentError ? error.code : undefined);
  }
  ...
  try {
    ...
    throw recoveryUnproven("invalid_receipt_response");
  }
  try {
    ...
    if (error instanceof DeviceEnrollmentError && error.code === DEVICE_ENROLLMENT_ERRORS.BINDING) throw recoveryUnproven("receipt_binding_failed");
    throw recoveryUnproven("invalid_receipt");
  }
}
...
function recoveryProven(receipt) {
  return new DeviceEnrollmentError(DEVICE_ENROLLMENT_ERRORS.RECOVERY_PROVEN, "Cloud proves this device enrollment completed, but the control trust response is unavailable; setup stopped for manual reconciliation", {
    recovery: "receipt_verified",
    receipt_key_id: receipt.key_id,
    receipt_statement_hash: receipt.statement_hash,
    device_key_epoch: receipt.statement.device_key_epoch,
    enrollment_id: receipt.statement.enrollment_id,
    organization_id: receipt.statement.organization_id,
    device_id: receipt.statement.device_id,
    key_fingerprint: receipt.statement.device_key_fingerprint,
    candidate_id: receipt.statement.candidate_id,
    challenge_nonce_digest: receipt.statement.challenge_nonce_digest
  });
}

function recoveryUnproven(observed = undefined) {
