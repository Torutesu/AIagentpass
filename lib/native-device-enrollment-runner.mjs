...
function trustedExecutable(filesystem, executable, applicationRoot, expectedOwner) {
  if (typeof executable !== "string" || !path.isAbsolute(executable) || executable === applicationRoot || !executable.startsWith(`${applicationRoot}${path.sep}`)) fail("UNSAFE_PATH", "Native enrollment service escapes the application boundary");
  ...
  while (true) {
    ...
    const leaf = current === executable;
    if ((leaf ? !stat.isFile() || (stat.mode & 0o111) === 0 : !stat.isDirectory()) || stat.isSymbolicLink() || stat.uid !== expectedOwner || (stat.mode & 0o022) !== 0 || (leaf && stat.nlink !== 1)) fail("UNSAFE_PATH", "Native enrollment service path is unsafe");
...
function defaultAuthenticate() {
  return spawnSync("/usr/bin/sudo", ["-v"], { stdio: ["inherit", "ignore", "inherit"], env: { PATH: "/usr/bin:/bin:/usr/sbin:/sbin" }, timeout: 120_000 });
}
function defaultRun(service, action, config, input) {
  return spawnSync("/usr/bin/sudo", ["-n", "--", service, "--device-auth", action, "--config", config], { encoding: "utf8", input, env: { PATH: "/usr/bin:/bin:/usr/sbin:/sbin" }, shell: false, timeout: 30_000, maxBuffer: MAX_OUTPUT });
}
function invoke(run, service, action, config, input, label) {
  const result = run(service, action, config, input);
  ...
  return parseCanonical(result.stdout, label);
}

/** Use the service-owned Secure Enclave device key through a root-only signed primitive. */
export function createNativeDeviceEnrollmentRunner(options = {}) {
  ...
  trustedExecutable(filesystem, service, applicationRoot, expectedOwner);
  if (typeof config !== "string" || !path.isAbsolute(config)) fail("UNSAFE_PATH", "Native service configuration path is invalid");
  let authenticated = false;
  const privilegedInvoke = (action, input, label) => {
    if (!authenticated) {
      const result = authenticate();
      if (!object(result) || result.error || result.signal || result.status !== 0) fail("PRIVILEGE_AUTHENTICATION_FAILED", "Administrator authentication failed");
      authenticated = true;
    }
    return invoke(run, service, action, config, input, label);
  };
  return Object.freeze({
    publicKey() {
      const value = privilegedInvoke("key", undefined, "device enrollment key");
    ...
    sign({ bytes } = {}) {
      ...
      const value = privilegedInvoke("sign", input, "device enrollment signing");
