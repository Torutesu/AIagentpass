...
export function removeIntegration(plan, { dryRun = true } = {}) {
  ...
  const pendingRecovery = inspectRemovalJournal(plan);
  if (pendingRecovery && dryRun) {
    return { ...plan, dryRun: true, changed: true, removed: false, owned: true, reason: "recovery_pending", atomic: false, content: pendingRecovery.content };
  }
  if (pendingRecovery) recoverRemovalJournal(plan, pendingRecovery);
  ...
  const { quarantine, replacement: temporary } = removalJournalPaths(plan.target);
  try {
    ...
    try { fs.linkSync(plan.target, quarantine); }
    catch { throw new Error(`Integration removal journal already exists or is competing: ${quarantine}`); }
    const quarantinedStat = fs.lstatSync(quarantine);
    if (quarantinedStat.nlink !== 2 || quarantinedStat.dev !== stat.dev || quarantinedStat.ino !== stat.ino || fs.readFileSync(quarantine, "utf8") !== current) {
      fs.unlinkSync(quarantine);
      throw new Error(`Integration file changed during removal: ${plan.target}`);
    }
    const live = fs.lstatSync(plan.target);
    if (live.dev !== quarantinedStat.dev || live.ino !== quarantinedStat.ino) {
      fs.unlinkSync(quarantine);
      throw new Error(`Integration file changed during journal reservation: ${plan.target}`);
    }
    fs.unlinkSync(plan.target);
    fsyncDirectory(path.dirname(plan.target));
    try {
      // link(2) provides an atomic no-replace installation boundary.
      fs.linkSync(temporary, plan.target);
    } catch {
      if (!lstatIfPresent(plan.target)) {
        try { fs.linkSync(quarantine, plan.target); fs.unlinkSync(quarantine); } catch {}
      }
      throw new Error(`Integration file changed during final installation: ${plan.target}`);
    }
...
function removalJournalPaths(target) {
  return {
...
function readJournalFile(file) {
  ...
  if (!stat.isFile() || stat.isSymbolicLink() || ![1, 2].includes(stat.nlink) || (uid !== undefined && stat.uid !== uid) || (stat.mode & 0o022) !== 0 || stat.size > 1024 * 1024) {
    throw new Error(`Integration removal journal is unsafe: ${file}`);
  }
  return fs.readFileSync(file, "utf8");
}

function inspectRemovalJournal(plan) {
  const paths = removalJournalPaths(plan.target);
  const hasQuarantine = Boolean(lstatIfPresent(paths.quarantine));
  const hasReplacement = Boolean(lstatIfPresent(paths.replacement));
  if (!hasQuarantine && !hasReplacement) return null;
  ensureRemovalParent(plan);
  const targetStat = lstatIfPresent(plan.target);
  const quarantineStat = hasQuarantine ? lstatIfPresent(paths.quarantine) : null;
  const reserved = Boolean(targetStat && quarantineStat && targetStat.dev === quarantineStat.dev && targetStat.ino === quarantineStat.ino);
  const target = targetStat ? (reserved ? readJournalFile(plan.target) : readSafeRemovalFile(plan.target)) : null;
  const original = hasQuarantine ? readJournalFile(paths.quarantine) : null;
  let expected = null;
  if (original !== null) {
    const rendered = renderRemoval(plan, original);
    if (!rendered.changed) throw new Error("Integration removal quarantine is not AgentPass-owned");
    expected = rendered.content;
    if (target !== null && !reserved && target !== expected) throw new Error("Concurrent integration changes require manual recovery");
  }
  if (hasReplacement) {
    const staged = readJournalFile(paths.replacement);
    if (expected !== null && staged !== expected) throw new Error("Integration removal journal is inconsistent");
    if (expected === null && target !== null) {
      const rendered = renderRemoval(plan, target);
      if (staged !== target && (!rendered.changed || staged !== rendered.content)) throw new Error("Integration removal replacement is inconsistent");
    }
    expected ??= staged;
  }
  return { ...paths, hasQuarantine, hasReplacement, target, reserved, content: expected };
}

function recoverRemovalJournal(plan, journal) {
  if (journal.hasQuarantine && journal.target === null) {
    try { fs.linkSync(journal.quarantine, plan.target); fs.unlinkSync(journal.quarantine); }
    catch { throw new Error("Concurrent integration changes prevented journal recovery"); }
  }
  else if (journal.hasQuarantine) fs.unlinkSync(journal.quarantine);
  if (journal.hasReplacement && lstatIfPresent(journal.replacement)) fs.unlinkSync(journal.replacement);
  fsyncDirectory(path.dirname(plan.target));
}
