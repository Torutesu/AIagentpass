...
function controlProvisioning(value) {
  exact(value, ["changed", "new_fingerprint", "old_fingerprint"], "control provisioning response");
  if (typeof value.changed !== "boolean") fail("INVALID_RESPONSE", "Control provisioning changed flag is invalid");
  fingerprint(value.new_fingerprint, "new control fingerprint");
  if (value.old_fingerprint !== null) fingerprint(value.old_fingerprint, "old control fingerprint");
  return value;
}
...
export function createNativeBootstrapRunner(options = {}) {
  ...
  return Object.freeze({
    ...
    provisionControl(input) {
      if (!object(input)) fail("INVALID_INPUT", "Control provisioning input is invalid");
      return controlProvisioning(parseCanonical(invoke(service, ["--provision-control", "--config", config], input, true, "control provisioning").stdout, "control provisioning"));
    }
  });
}
