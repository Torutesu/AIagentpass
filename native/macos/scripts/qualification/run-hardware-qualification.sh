...
for value in "$AGENTPASS_LAUNCHD_HOST_CHILD_PROBE" "$AGENTPASS_NSXPC_PROBE" "$AGENTPASS_CRASH_RESTART_PROBE"; do
  [[ -f "$value" && -x "$value" && ! -L "$value" ]] || { echo "fixed probes must be regular non-symlink executables" >&2; exit 1; }
  [[ "$(stat -f '%Su' "$value")" == root ]] || { echo "fixed probes must be root-owned" >&2; exit 1; }
  mode="$(stat -f '%Lp' "$value")"
  case "$mode" in [2367][0-7][0-7]|[0-7][2367][0-7]|[0-7][0-7][2367]) echo "fixed probes must not be writable" >&2; exit 1;; esac
done
...
check_probe_binding() {
  ...
  [[ -n "$digest" || -n "$identity" ]] || { echo "$label has no protected digest or signing identity binding" >&2; exit 1; }
  [[ -z "$digest" || "$digest" =~ ^[0-9a-f]{64}$ ]] || { echo "$label expected SHA-256 is invalid" >&2; exit 1; }
