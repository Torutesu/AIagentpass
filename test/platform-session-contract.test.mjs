...
test("session and revoke HTTP envelopes keep bearer and internal identity out of JSON", () => {
  ...
  assert.deepEqual(Object.keys(sessionSchema.properties).sort(), [
    "authenticated_at", "capability", "expires_at", "issued_at", "operation", "session_id", "status"
  ]);
  assert.equal(sessionSchema.minProperties, 7);
  assert.equal(sessionSchema.maxProperties, 7);
  const forbidden = [
    "version", "type", "principal_id", "assignment_id", "authority_generation", "request_digest_sha256",
    "credential_id", "credential_id_sha256", "member_id", "organization_id", "session_bearer", "session_token", "access_token"
  ];
  ...
  for (const name of ["version", "type", "principal_id", "assignment_id", "authority_generation", "request_digest_sha256"]) {
    assertInvalid("platform-session-response-v1.schema.json", { ...sessionMetadata(), [name]: "internal" });
  }
...
test("cookie, CSRF, origin, no-store, and operation surface are explicit", () => {
  ...
  assert.equal(openapi.components.responses.TooManyRequests.headers["Retry-After"].$ref, "#/components/headers/RetryAfter");
...
test("catalog records every platform-session schema and its public HTTP references", () => {
  ...
  for (const id of expected) {
    ...
    if (id === "platform-session-response-v1") {
      assert.deepEqual(entry.actor_binding, { required: false, paths: [] });
    }
  }
});
