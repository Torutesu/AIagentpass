...
const fixture = () => {
  ...
  const env = { AGENTPASS_P0C_ARTIFACT_PATH: artifact, AGENTPASS_P0C_ARTIFACT_SHA256: digest(bytes), AGENTPASS_P0C_SOURCE_COMMIT: 'a'.repeat(40), AGENTPASS_P0C_TEAM_ID: 'A1B2C3D4E5', AGENTPASS_P0C_GATE: 'gatekeeper-notarization', AGENTPASS_P0C_TESTS_JSON: '["exact-pkg-install"]' };
...
test('scenario success requires exact declaration, release binding, and explicit proof list', async () => {
  ...
  assert.deepEqual(result, { schema_version: 1, gate: 'gatekeeper-notarization', status: 'passed', tests: [{ name: 'exact-pkg-install', status: 'passed' }], bindings: { artifact_sha256: value.bindings.artifactSha256, source_commit: 'a'.repeat(40), team_id: 'A1B2C3D4E5' } });
