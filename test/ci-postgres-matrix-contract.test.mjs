...
test("N1 runs an isolated PostgreSQL 16/17 matrix with a fresh head-51 database", () => {
  ...
  assert.match(section, /assert\.equal\(result\.currentVersion, 51\)/u);
  assert.match(section, /assert\.equal\(history\.rowCount, 51\)/u);
  assert.match(section, /Array\.from\(\{ length: 51 \}/u);
...
test("N1 bootstraps roles, migrates as the actual migrator login, then reconciles and checks privileges", () => {
  ...
  const migration = section.indexOf("Run fresh migrations through head 51");
...
test("N1 executes the 0048-0051 qualification set and rejects unexpected skips", () => {
  const section = job("postgres-authority-matrix");
...
test("N1 evidence is source-SHA-bound and uploaded with a fail-closed artifact contract", () => {
  ...
  assert.match(section, /migration_head: 51/u);
  ...
  assert.match(section, /report_sha256: sha256\(upgradeOutput\)/u);
