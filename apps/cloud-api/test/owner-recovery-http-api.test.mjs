...
import { OWNER_RECOVERY_ERROR_CODES, OWNER_RECOVERY_OPERATIONS } from "../src/human-auth/recovery/service.mjs";
import {
...
function makeApi({ abuseControls = undefined, serviceOverrides = {} } = {}) {
  const calls = [];
  const service = {
...
test("checks every fixed recovery operation before service work and keeps exchange identity secret-free", async () => {
  const checks = [];
  const { api } = makeApi({ abuseControls: {
    async check(input) { checks.push(input); return { allowed: true }; },
    async checkAnonymous(input) { checks.push(input); return { allowed: true }; }
  } });
  ...
  assert.equal(JSON.stringify(exchangeCheck).includes("E".repeat(43)), false);
  assert.match(exchangeCheck.principalId, /^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/u);
  assert.equal(Object.hasOwn(exchangeCheck, "organizationId"), false);
});

test("maps shared abuse denial to 429 with Retry-After and outage to stable 503", async () => {
  const denied = makeApi({ abuseControls: { async check() { throw new HumanAuthAbuseControlError(HUMAN_AUTH_ABUSE_ERROR_CODES.RATE_LIMITED, { retryAfterSeconds: 7, cause: new Error("secret provider detail") }); }, async checkAnonymous() { return { allowed: true }; } } });
  ...
  const unavailable = makeApi({ abuseControls: { async check() { return { allowed: true }; }, async checkAnonymous() { throw new Error("database password=secret"); } } });
...
test("maps idempotency key reuse to a stable secret-free 409", async () => {
  const { api } = makeApi({ serviceOverrides: { async create() { throw Object.assign(new Error("database payload secret"), { code: OWNER_RECOVERY_ERROR_CODES.IDEMPOTENCY_CONFLICT }); } } });
  const result = await api.handle(request(OWNER_RECOVERY_HTTP_PATHS.requests(ORG), { body: {}, headers: { ...normalHeaders, "content-type": "application/json", "idempotency-key": "create-conflict" } }));
  assert.equal(result.status, 409, JSON.stringify(result.body));
  assert.equal(result.body.error.code, OWNER_RECOVERY_HTTP_ERROR_CODES.IDEMPOTENCY_CONFLICT);
