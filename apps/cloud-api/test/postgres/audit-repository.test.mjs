...
test("PostgreSQL audit repository uses tenant-qualified immutable tuple ordering and limit+1", async () => {
  ...
  const first = await repository.listDeviceAuditEvents({ organization_id: organizationId, device_id: deviceA, limit: 2 });
  assert.equal(first.events.length, 2);
  assert.ok(first.next_cursor);
  assert.match(calls[0].text, /WHERE organization_id = \$1/);
  assert.match(calls[0].text, /ORDER BY \(redacted_json ->> 'device_timestamp'\) DESC,device_id DESC,event_id DESC/);
  assert.equal(calls[0].params.at(-1), 3);

  const second = await repository.listDeviceAuditEvents({ organization_id: organizationId, device_id: deviceA, limit: 2, cursor: first.next_cursor });
  assert.equal(second.events.length, 1);
  assert.equal(second.next_cursor, null);
  assert.match(calls[1].text, /\(\(redacted_json ->> 'device_timestamp'\), device_id, event_id\) < \(\$3, \$4::uuid, \$5::uuid\)/);
  assert.deepEqual(calls[1].params.slice(0, 5), [organizationId, deviceA, first.events.at(-1).event.device_timestamp, first.events.at(-1).device_id, first.events.at(-1).event_id]);

  await assert.rejects(() => repository.listDeviceAuditEvents({ organization_id: organizationId, device_id: deviceB, cursor: first.next_cursor }), /invalid/);
  await assert.rejects(() => repository.listDeviceAuditEvents({ organization_id: organizationId, device_id: deviceA, cursor: `${first.next_cursor.slice(0, -1)}${first.next_cursor.endsWith("A") ? "B" : "A"}` }), /invalid/);
});
...
test("PostgreSQL audit repository fails closed on inconsistent stored event timestamps", async () => {
  ...
  await assert.rejects(() => repository.listDeviceAuditEvents({ organization_id: organizationId, device_id: deviceA }), PostgresAuditRepositoryError);
});
