...
test("0061 remains the cataloged forward-only hosted OAuth output qualification migration", async () => {
  ...
  assert.ok(POSTGRES_SCHEMA_HEAD.version >= 61);
  assert.equal(POSTGRES_SCHEMA_HEAD.migrations.find(({ version }) => version === 61)?.name, "0061_hosted_oauth_output_qualification.sql");
  assert.match(sql, /^BEGIN;[\s\S]*COMMIT;\s*$/u);
  assert.doesNotMatch(sql, /DROP\s+(?:TABLE|COLUMN|INDEX)|TRUNCATE\s+TABLE/iu);
  const entry = catalog.entries.find(({ id }) => id === "migration.0061_hosted_oauth_output_qualification");
  assert.equal(entry?.version, 61);
});
