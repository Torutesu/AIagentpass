...
export const createMaintenanceService = ({ profile = "hosted", repository, provider, clock, uuid } = {}) => {
  if (!["hosted", "test"].includes(profile) || (profile === "hosted" && [repository, provider, clock, uuid].some(isTestOnlyDependency))) fail(MAINTENANCE_ERROR_CODES.INVALID_CONFIGURATION);
