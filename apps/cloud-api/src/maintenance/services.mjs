...
import { maintenanceRepository, maintenanceProvider, maintenanceClock, maintenanceUuid, isTestOnlyDependency, isRegisteredHostedDependency } from "./interfaces.mjs";
...
export const createMaintenanceService = ({ profile = "hosted", repository, provider, clock, uuid } = {}) => {
  if (!["hosted", "test"].includes(profile) || (profile === "hosted" && ([repository, provider, clock, uuid].some(isTestOnlyDependency) || [repository, provider, clock, uuid].some((dependency) => !isRegisteredHostedDependency(dependency))))) fail(MAINTENANCE_ERROR_CODES.INVALID_CONFIGURATION);
