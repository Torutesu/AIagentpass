...
import { maintenanceRepository, maintenanceProvider, maintenanceClock, maintenanceUuid, isTestOnlyDependency, HOSTED_ADAPTER_BRAND } from "./interfaces.mjs";
...
export const createMaintenanceService = ({ profile = "hosted", repository, provider, clock, uuid } = {}) => {
  if (!["hosted", "test"].includes(profile) || (profile === "hosted" && ([repository, provider, clock, uuid].some(isTestOnlyDependency) || [repository, provider, clock, uuid].some((dependency) => dependency?.[HOSTED_ADAPTER_BRAND] !== true)))) fail(MAINTENANCE_ERROR_CODES.INVALID_CONFIGURATION);
