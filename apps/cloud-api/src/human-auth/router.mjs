...
export function createHumanAuthRouter({ sessionApi, webauthnApi, registrationApi, managementApi, organizationApi, recoveryApi, recoveryDeadLetterApi, agentSessionGrantApi, qualificationGrantBatchApi } = {}) {
  ...
  async function handle(input) {
    ...
    if (url.pathname === SESSION_RESUME_PATH && !url.search && !url.hash) {
      return sessionApi.handle(cloneRequest(input, "/session/resume"));
    }
    ...
    if (isOwnerRecoveryPath(url)) {
      ...
      return recoveryApi.handle(cloneRequest(input, `${url.pathname}${url.search}`));
    }
    ...
    if (isQualificationGrantBatchPath(url)) {
      if (!qualificationGrantBatchApi) return response(404, { error: { code: "not_found", message: "Resource not found" } });
      return qualificationGrantBatchApi.handle(cloneRequest(input, `${url.pathname}${url.search}`));
    }
  ...
  return Object.freeze({ handle, paths: Object.freeze({ session: SESSION_PATH, resume: SESSION_RESUME_PATH, options: OPTIONS_PATH, verify: VERIFY_PATH, registrationOptions: REGISTRATION_OPTIONS_PATH, registrationVerify: REGISTRATION_VERIFY_PATH, organizations: ORGANIZATIONS_PATH, acceptInvitation: ACCEPT_INVITATION_PATH }) });
}
...
function isQualificationGrantBatchPath(url) {
  return !url.hash && QUALIFICATION_GRANT_BATCH_ROUTE.test(url.pathname);
}
