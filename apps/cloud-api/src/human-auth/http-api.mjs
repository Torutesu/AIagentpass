...
function mapServiceError(error, phase) {
  ...
  const diagnostic = phase === "begin" && typeof error?.storageCode === "string"
    ? `${String(error.code).toLowerCase()}_${error.storageCode}`
    : phase === "begin" && typeof error?.code === "string" && /^ERR_WEBAUTHN_[A-Z_]+$/u.test(error.code)
