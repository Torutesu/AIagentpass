...
export function createHumanAuthRuntime({ postgresRuntime, tokenRecords, origin, rpId, now = () => Date.now() } = {}) {
  ...
  const consoleIdentity = createConsoleIdentityAdapter({ tokenRecords, now });
  ...
  const api = createHumanAuthRouter({ sessionApi, webauthnApi });
  return Object.freeze({ api, humanSession, recentAuthService, ceremony, sessionApi, webauthnApi, allowedOperations: ALLOWED_RECENT_AUTH_OPERATIONS });
}
