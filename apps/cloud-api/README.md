...
Hosted additionally requires `AGENTPASS_CAPABILITY_NONCE_SECRET`, one canonical unpadded 32-byte base64url secret shared by all Cloud instances. It derives retry-stable capability nonces with a purpose-separated HMAC; PostgreSQL stores only the nonce digest and safe statement metadata. Rotate it only after all outstanding capability idempotency windows have expired. Hosted device request nonces and authenticated organization rate limits are consumed atomically in PostgreSQL, so instance switching and process restart cannot reset those controls. Pre-authentication admission remains transport-scoped and does not grant authority.
