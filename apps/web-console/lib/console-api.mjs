...
function errorResponse(error) {
  if (error instanceof CloudResponseError) {
    ...
    if (process.env.P0B_LIVE_BROWSER === "1") process.stderr.write(`P0B_DIAGNOSTIC_CONSOLE_UPSTREAM_ERROR code=${code}\n`);
    if (typeof source.request_id === "string" && /^[A-Za-z0-9._:-]{1,128}$/.test(source.request_id)) responseBody.request_id = source.request_id;
    return jsonResponse(error.status, responseBody, { "x-agentpass-error-code": code, ...(process.env.P0B_LIVE_BROWSER === "1" ? { "x-agentpass-error-source": "cloud" } : {}) });
  }
  if (error instanceof ConsoleApiError) {
    if (process.env.P0B_LIVE_BROWSER === "1") process.stderr.write(`P0B_DIAGNOSTIC_CONSOLE_ERROR code=${error.code}\n`);
    return jsonResponse(error.status, { error: { code: error.code, message: error.message } }, { "x-agentpass-error-code": error.code, ...(process.env.P0B_LIVE_BROWSER === "1" ? { "x-agentpass-error-source": "console" } : {}) });
  }
  if (process.env.P0B_LIVE_BROWSER === "1") process.stderr.write("P0B_DIAGNOSTIC_CONSOLE_INTERNAL_ERROR code=internal_error\n");
  return jsonResponse(500, { error: { code: "internal_error", message: "Internal error" } }, { "x-agentpass-error-code": "internal_error", ...(process.env.P0B_LIVE_BROWSER === "1" ? { "x-agentpass-error-source": "console" } : {}) });
}
