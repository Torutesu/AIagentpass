...
export function SecurityPanel({ onSessionExpired, onSessionSignedOut, securityClient: injectedClient }: SecurityPanelProps) {
  ...
  const load = useCallback(async (signal?: AbortSignal, clearMessages = false): Promise<boolean> => {
  ...
  return <section className="security-panel" aria-labelledby="security-panel-title">
    ...
    <article className="security-panel__card">
      <div className="security-panel__title-row">
        ...
        <div className="security-panel__actions"><button className="secondary-button" type="button" disabled={busy || loadState === "loading"} onClick={addPasskey}>{actionKey === "passkey:add" ? "登録中…" : "パスキーを追加"}</button><button className="secondary-button" type="button" disabled={busy || loadState === "loading"} onClick={() => void load(undefined, true)}>再読み込み</button></div>
      </div>
      {loadState === "loading" ? <p className="section-note" role="status">セキュリティ情報を読み込み中…</p> : loadState === "error" ? <RetryState message={error} onRetry={() => void load(undefined, true)} /> : passkeys.length === 0 ? <EmptyState title="登録済みのパスキーはありません" copy="パスキーを追加すると、次回から安全に再認証できます。" /> : <>
