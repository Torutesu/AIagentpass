...
The service still refuses remote-control policies because protected sequence persistence is not implemented. The repository also does not ship the Developer ID-signed, provisioned, notarized app bundle required to activate the code-signing and keychain boundary. Native P-256 checkpoints currently require independent retention and are not accepted by the Ed25519 reference anchor. Until packaging and protected control state are complete, AgentPass must not claim a production-ready hardened boundary.
