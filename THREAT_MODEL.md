...
Version 0.10 ships the source-level Swift/XPC core for this mode. The service holds separate non-exportable Secure Enclave P-256 signing and checkpoint keys, gates clients by UID and code-signing requirement, and independently verifies Agent signatures, replay state, root-owned policy, Git context, commit tree, and parents. Signing allow, deny, and error outcomes enter a root-owned, fsync-backed SHA-256 audit chain. A separate P-256 key signs chained checkpoints; startup, health, append, and checkpoint operations fail closed if the local chain, checkpoint signatures, ownership, or permissions are invalid.

The service still refuses session-required and remote-control policies because those protected native state machines are not implemented. The repository also does not ship the Developer ID-signed, provisioned, notarized app bundle required to activate the code-signing and keychain-access-group boundary. Native P-256 checkpoints currently require independent retention and are not accepted by the Ed25519 reference anchor. Until packaging and protected control/session state are complete, AgentPass must not claim a production-ready hardened boundary.
