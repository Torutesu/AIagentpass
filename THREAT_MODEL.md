...
Version 0.13 ships the source-level Swift/XPC core for this mode. The service holds separate non-exportable Secure Enclave P-256 signing and checkpoint keys, gates clients by UID and code-signing requirement, and independently verifies Agent signatures, protected sessions, signed remote control, replay state, root-owned policy, Git context, commit tree, and parents. It periodically retrieves control bundles from a credential-free HTTPS URL fixed in root-owned configuration, rejects redirects and responses over 256 KiB, and never replaces active state until signature and sequence checks pass. Session issuance uses a one-time service challenge signed by a separately pinned Secure Enclave key whose private-key use requires human presence. The service stores only token hashes in memory, binds them to one Agent and TTL, and clears sessions and pending approvals on restart, explicit revocation, or any accepted higher control sequence.
...
The native service requires a signed initial bundle when remote control is configured, checks it before signing and session issuance, and persists accepted higher sequences atomically. Network failure leaves the last signed state active only until its signed expiry; expiry then stops signing. The repository does not ship the Developer ID-signed, provisioned, notarized app bundle required to activate the code-signing and keychain boundary. Native P-256 checkpoints currently require independent retention and are not accepted by the Ed25519 reference anchor. Until signed packaging and hardware integration testing are complete, AgentPass must not claim a production-ready hardened boundary.
