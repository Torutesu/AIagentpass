...
Version 0.12 ships the source-level Swift/XPC core for this mode. The service holds separate non-exportable Secure Enclave P-256 signing and checkpoint keys, gates clients by UID and code-signing requirement, and independently verifies Agent signatures, protected sessions, signed remote control, replay state, root-owned policy, Git context, commit tree, and parents. Session issuance uses a one-time service challenge signed by a separately pinned Secure Enclave key whose private-key use requires human presence. The service stores only token hashes in memory, binds them to one Agent and TTL, and clears sessions and pending approvals on restart or explicit revocation.
...
The native service requires a valid initial signed bundle when remote control is configured, checks it before signing and session issuance, and persists accepted higher sequences atomically. Bundle retrieval is currently an operator or external-scheduler action rather than a service-owned background task. The repository also does not ship the Developer ID-signed, provisioned, notarized app bundle required to activate the code-signing and keychain boundary. Native P-256 checkpoints currently require independent retention and are not accepted by the Ed25519 reference anchor. Until signed packaging and hardware integration testing are complete, AgentPass must not claim a production-ready hardened boundary.
