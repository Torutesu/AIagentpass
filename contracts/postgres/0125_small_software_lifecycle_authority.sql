...
BEGIN
 IF public.agentpass_current_organization_id() IS DISTINCT FROM p_organization_id THEN RAISE EXCEPTION USING ERRCODE='insufficient_privilege',MESSAGE='route tenant is invalid'; END IF;
 ...
 IF NOT EXISTS (SELECT 1 FROM public.small_software_releases r JOIN public.small_software_approvals a ON a.organization_id=r.organization_id AND a.release_id=r.id WHERE r.organization_id=p_organization_id AND r.id=p_release_id AND r.app_id=p_app_id AND r.state IN ('approved','deploying') AND a.state='approved' AND a.artifact_digest=(SELECT artifact_digest FROM public.small_software_releases WHERE organization_id=p_organization_id AND id=p_release_id)) THEN RAISE EXCEPTION USING ERRCODE='check_violation',MESSAGE='release approval binding is missing'; END IF;
...
BEGIN
 IF public.agentpass_current_organization_id() IS DISTINCT FROM p_organization_id THEN RAISE EXCEPTION USING ERRCODE='insufficient_privilege',MESSAGE='lifecycle tenant is invalid'; END IF;
