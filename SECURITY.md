...
The default broker is a per-user macOS LaunchAgent. It does not isolate its files or key reference from malware with arbitrary code execution as that same user. Git pre-push hooks are also bypassable. Use least-privilege repository access and server-side branch protection, and read [THREAT_MODEL.md](THREAT_MODEL.md) before deploying. Version 0.16 includes the Swift/XPC and Secure Enclave source for the stronger boundary, protected sessions and control state, bounded jittered control refresh, a root-owned native audit and remote-receipt chain, and a signed/notarizable app build pipeline with a Service Management registrar. The repository still does not publish a Developer ID-signed production artifact; see [docs/NATIVE_BROKER.md](docs/NATIVE_BROKER.md).
