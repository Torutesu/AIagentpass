...
export function validateHostedIdentityBootstrapContract(document, { root = DEFAULT_ROOT } = {}) {
  ...
  if (isObject(document)) {
    ...
    if (document.activation?.prerequisite_forward_migration !== "0056_identity_epoch_invalidation" || document.activation?.planned_forward_migration !== "0057_hosted_identity_bootstrap") fail("migration ordering is not pinned after identity epoch invalidation");
