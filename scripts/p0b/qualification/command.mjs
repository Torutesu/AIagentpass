...
export function runQualificationCommand(command, args, options) {
  ...
  const observeDiagnostics = (chunk, stream) => {
    ...
    for (const match of candidate.toString("utf8").matchAll(/P0B_DIAGNOSTIC_(?:CLOUD_READINESS state=(?:ready|unavailable)|KEYBOARD_OPTIONS code=[a-z][a-z0-9_]{0,63}|SUMMARY_CODE code=(?:[a-z][a-z0-9_]{0,63}|none)|SUMMARY_STATUS status=\d{3}|SUMMARY_HEADER status=\d{3} code=(?:[a-z][a-z0-9_]{0,63}|none) content_type=(?:json|other)|SUMMARY_REFRESH code=[A-Za-z0-9_.:-]{1,96}|SUMMARY_PARSE path=[.$\w\[\]]{1,128} reason=[a-z_]{1,64}|SUMMARY_RESPONSES statuses=\d{3}(?:,\d{3}){0,7}|STAGE_TRACE stages=[A-Z][A-Z0-9_]{1,47}(?:,[A-Z][A-Z0-9_]{1,47}){0,7})/gu)) {
