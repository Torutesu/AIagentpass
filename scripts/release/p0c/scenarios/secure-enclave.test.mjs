...
const machine = { serviceConfigPath: '/Library/Application Support/AgentPass/native-service.json', serviceLabel: 'dev.agentpass.native-service', executables: { native_service: { path: '/service', sha256: 'c'.repeat(64) }, native_client: { path: '/client', sha256: 'd'.repeat(64) } } };
...
test('secure enclave scenario proves live nonexportable key, possession, and control-key continuity', async () => {
  assert.deepEqual(await performSecureEnclaveEnrollment({ release, machine, production: false, getUid: () => 0, runPinned: runner() }), ['secure-enclave-key-creation', 'secure-enclave-nonexportability']);
});

test('secure enclave scenario rejects exportability, bad signatures, key substitution, and non-root execution', async () => {
  await assert.rejects(() => performSecureEnclaveEnrollment({ release, machine, production: false, getUid: () => 0, runPinned: runner({ snapshotValue: snapshot({ private_exportable: true }) }) }), /did not prove/);
  await assert.rejects(() => performSecureEnclaveEnrollment({ release, machine, production: false, getUid: () => 0, runPinned: runner({ corruptSignature: true }) }), /signature is invalid/);
  await assert.rejects(() => performSecureEnclaveEnrollment({ release, machine, production: false, getUid: () => 0, runPinned: runner({ controlValue: control({ control_device_auth_public_key: 'substituted' }) }) }), /not bound/);
  await assert.rejects(() => performSecureEnclaveEnrollment({ release, machine, production: false, getUid: () => 501, runPinned: runner() }), /requires root/);
});
