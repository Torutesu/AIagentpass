...
if (!manifest.artifacts.some((artifact) => artifact.role === 'product')) throw new Error('release manifest has no product artifact');
...
if (!safeName(checksums.name) || names.has(checksums.name) || !Number.isSafeInteger(checksums.bytes) || checksums.bytes <= 0 || !validDigest(checksums.sha256) || !Number.isSafeInteger(checksums.entry_count) || checksums.entry_count !== manifest.artifacts.length + notarization.evidence.length) throw new Error('invalid checksums evidence');
const checksumEntries = [...manifest.artifacts, ...notarization.evidence].sort((a, b) => lexicalCompare(a.name, b.name));
