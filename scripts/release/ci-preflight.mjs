...
export function assertWorkflowBoundary({ eventName, ref, repository } = {}) {
  if (eventName === "pull_request") throw new CiPreflightError("release preflight cannot access release credentials from pull requests");
  if (ref !== "refs/heads/main") throw new CiPreflightError("release preflight requires protected main");
  if (repository !== "Torutesu/AIagentpass") throw new CiPreflightError("repository is not canonical");
  return true;
}

export function assertCleanCheckout(repoRoot) {
  const status = execFileSync("git", ["-C", repoRoot, "status", "--porcelain=v1", "--untracked-files=all"], { encoding: "utf8" }).trim();
  if (status) throw new CiPreflightError(`checkout is dirty:\n${status}`);
  return true;
}

export function assertCandidateBinding({ repoRoot, manifestPath, expectedCommit, expectedTree, sourceRef = "HEAD" } = {}) {
  const manifest = readJson(manifestPath);
  if (manifest?.source?.commit !== expectedCommit || manifest?.source?.tree !== expectedTree) throw new CiPreflightError("candidate source commit/tree differs from the signed candidate");
  const head = execFileSync("git", ["-C", repoRoot, "rev-parse", `${sourceRef}^{commit}`], { encoding: "utf8" }).trim();
  const tree = execFileSync("git", ["-C", repoRoot, "rev-parse", `${sourceRef}^{tree}`], { encoding: "utf8" }).trim();
  if (head !== expectedCommit || tree !== expectedTree) throw new CiPreflightError("checkout HEAD/tree differs from the expected candidate");
  const product = Array.isArray(manifest.artifacts) ? manifest.artifacts.filter((item) => item?.role === "product") : [];
  if (product.length !== 1 || typeof product[0].name !== "string" || product[0].name.length === 0
    || path.isAbsolute(product[0].name) || path.posix.basename(product[0].name) !== product[0].name
    || product[0].name === "." || product[0].name === ".."
    || !/^[0-9a-f]{64}$/u.test(product[0].sha256)
    || manifest.candidate_id !== `release-pkg-sha256-v1-${product[0].sha256}`) {
    throw new CiPreflightError("candidate_id is not bound to one safe product artifact digest");
  }
  const artifactPath = path.join(path.dirname(path.resolve(manifestPath)), product[0].name);
  let artifactStat;
  try { artifactStat = fs.lstatSync(artifactPath); } catch { throw new CiPreflightError("product artifact is not readable"); }
  if (!artifactStat.isFile() || artifactStat.isSymbolicLink() || artifactStat.nlink !== 1) throw new CiPreflightError("product artifact is not a regular, non-linked file");
  const bytes = fs.readFileSync(artifactPath);
  if (bytes.length !== product[0].bytes || crypto.createHash("sha256").update(bytes).digest("hex") !== product[0].sha256) throw new CiPreflightError("product digest differs from the signed manifest");
  return { sourceCommit: expectedCommit, sourceTree: expectedTree, artifactSha256: product[0].sha256 };
}

export function assertExternalGates({ gateFiles = [], required = {} } = {}) {
  if (!Array.isArray(gateFiles) || gateFiles.length === 0) throw new CiPreflightError("no external gate evidence was supplied");
  const documents = gateFiles.map((file) => readJson(file));
  if (documents.some((document) => JSON.stringify(document).includes("not_proven"))) throw new CiPreflightError("external gate evidence contains not_proven");
  for (const [key, wanted] of Object.entries(required)) if (!documents.some((document) => document?.[key] === wanted)) throw new CiPreflightError(`required external gate ${key}=${wanted} was not proven`);
  return { files: gateFiles.length, required: Object.keys(required) };
}

export function assertProtectedArtifactSecretScan({ artifactPath, expectedSha256 } = {}) {
  const bytes = fs.readFileSync(path.resolve(artifactPath));
  const digest = crypto.createHash("sha256").update(bytes).digest("hex");
  if (digest !== expectedSha256) throw new CiPreflightError("digest is not manifest-bound");
  if (SECRET_MARKERS.some((marker) => marker.test(bytes.toString("utf8")))) throw new CiPreflightError("secret scan found secret material");
  return { status: "passed", findings: [], sha256: digest, bytes: bytes.length };
}

export function assertProtectedArtifactDirectory({ directory } = {}) {
  const result = scanProtectedArtifacts([path.resolve(directory)]);
  return { status: "passed", files: result.files.length, bytes: result.files.reduce((total, file) => total + file.bytes, 0) };
}
...
export function parseArguments(args) {
  ...
  const gateFiles = [];
  const required = {};
  const options = new Set(["--repo-root", "--manifest", "--expected-commit", "--expected-tree", "--source-ref", "--event-name", "--ref", "--repository", "--gate-file", "--require"]);
  for (let index = 0; index < args.length; index += 2) {
    const name = args[index];
    if (!options.has(name)) throw new CiPreflightError(`unknown argument ${name}`);
    const value = args[index + 1];
    if (value === undefined || value.startsWith("--")) throw new CiPreflightError(`argument ${name} requires a value`);
    if (name === "--gate-file") gateFiles.push(value);
    else if (name === "--require") { const separator = value.indexOf("="); if (separator <= 0) throw new CiPreflightError("argument --require requires key=value"); required[value.slice(0, separator)] = value.slice(separator + 1) === "true" ? true : value.slice(separator + 1) === "false" ? false : value.slice(separator + 1); }
    else values[name.slice(2).replace(/-([a-z])/g, (_, character) => character.toUpperCase())] = value;
  }
...
export function assertTerminalResults(results, { expectedSha, expectedRunId, expectedRunAttempt, expectedRepository, expectedWorkflow } = {}) {
  if (!Array.isArray(results) || results.length !== EXACT_CI_LANES.length) throw new CiPreflightError("CI terminal results contain exactly six lanes");
...
export function assertGithubCiRun(run, jobs, { expectedSha, repository = undefined, expectedRunId = undefined, expectedRunAttempt = undefined, workflowPath = ".github/workflows/ci.yml", expectedBranch = "main", jobsTotalCount = undefined } = {}) {
 ...
 */
export function assertCloudSignerKmsQualified(input, { expectedSourceCommit, expectedSourceTree, expectedDeploymentDigest, expectedArtifactSha256, expectedRunId, expectedJobId } = {}) {
  ...
  let result;
  try { result = verifyCloudSignerKmsQualificationEvidence(input, { expectedSourceCommit, expectedSourceTree, expectedDeploymentDigest, expectedArtifactSha256, expectedRunId, expectedJobId }); }
...
export function assertExternalQualificationEvidence(input, {
  ...
  if (JSON.stringify(input).includes("not_proven")) throw new CiPreflightError("external qualification evidence contains not_proven");
...
function assertExternalGate(name, gate, contract, release, { expectedArtifactSha256, expectedJobId, executionArtifacts } = {}) {
  ...
  if (gate.status === "not_run") {
    if (gate.reason === null || typeof gate.reason !== "string" || !EXTERNAL_REASON.has(gate.reason)
      || gate.execution !== null || !Array.isArray(gate.checks) || gate.checks.length !== 0) {
      throw new CiPreflightError(`external qualification gate ${name} not_run evidence is not fail-closed`);
    }
    return;
  }
  if (gate.reason !== null && (typeof gate.reason !== "string" || !EXTERNAL_REASON.has(gate.reason))) throw new CiPreflightError(`external qualification gate ${name} reason is invalid`);
...
function requireExternalBindingExpectation({ expectedRepository, expectedSourceCommit, expectedSourceTree, expectedReleaseArtifactSha256, expectedCiRunId, expectedCiRunAttempt }) {
  if (typeof expectedRepository !== "string" || !/^[^/\s]+\/[^/\s]+$/u.test(expectedRepository)
    ...
    || !GITHUB_ID.test(String(expectedCiRunId ?? "")) || !GITHUB_ATTEMPT.test(String(expectedCiRunAttempt ?? ""))) {
...
function walk(target, seenInodes, files, maximumBytes) {
  const stat = fs.lstatSync(target);
  if (stat.isSymbolicLink()) throw new CiPreflightError(`protected artifact contains a symlink: ${target}`);
  if (!stat.isDirectory() && !stat.isFile()) throw new CiPreflightError(`protected artifact contains an unsupported file: ${target}`);
  if (stat.isDirectory()) {
  ...
  if (SECRET_MARKERS.some((marker) => marker.test(text))) throw new CiPreflightError(`protected artifact secret scan found secret material: ${target}`);
...
function canonicalLane(value) {
  ...
  if (displayNames[value] !== undefined) return displayNames[value];
...
function usage() {
  return [
    "usage:",
    "  ci-preflight.mjs github <run.json> <jobs.json> <expected-sha> [repository] [expected-run-id] [expected-run-attempt]",
...
export async function runCli(argv = process.argv.slice(2)) {
  ...
  if (command === "github") {
    if (args.length < 3 || args.length > 6) throw new CiPreflightError(usage());
    ...
    return assertGithubCiRun(run, jobsEnvelope.jobs, { expectedSha: args[2], repository: args[3], expectedRunId: args[4], expectedRunAttempt: args[5], jobsTotalCount: jobsEnvelope.total_count });
  }
  ...
  if (command === "kms-qualification") {
    if (args.length !== 6 && args.length !== 7) throw new CiPreflightError(usage());
    return assertCloudSignerKmsQualified(readCanonicalJson(args[0]), {
      expectedSourceCommit: args[1], expectedSourceTree: args[2], expectedDeploymentDigest: args[3], ...(args.length === 7 ? { expectedArtifactSha256: args[4], expectedRunId: args[5], expectedJobId: args[6] } : { expectedRunId: args[4], expectedJobId: args[5] })
    });
  }
  ...
  if (command === "external-qualification") {
    ...
    const result = assertExternalQualificationEvidence(readCanonicalJson(args[0]), {
    ...
    if (result.status !== "passed" || result.qualified !== true) throw new CiPreflightError("external qualification is not promotion-qualified");
    return result;
  }
